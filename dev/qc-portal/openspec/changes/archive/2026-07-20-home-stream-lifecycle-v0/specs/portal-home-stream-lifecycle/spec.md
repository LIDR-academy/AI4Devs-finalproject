...
The Home page (`/`) SHALL fetch `GET /streams` exactly once on load with no polling, and render each returned stream showing its `title`. `description` SHALL be received and held client-side but SHALL NOT be displayed in v0. The order of the received array SHALL be preserved and no ordering SHALL be assumed. A **Start streaming** primary action SHALL always be present.

#### Scenario: Streams are live
- **WHEN** the visitor opens `/` and `GET /streams` returns two streams
- **THEN** both titles are rendered in the received order and the Start streaming action is visible

#### Scenario: Descriptions are not shown
- **WHEN** the visitor opens `/` and a returned stream carries a non-empty `description`
- **THEN** only the `title` is displayed and the `description` is not rendered anywhere on Home
...
The Start streaming action SHALL open a single modal containing a required `title` field and an optional `description` field, headed "Are you sure to start stream?" with a primary **Start** and a secondary **Cancel**. The modal SHALL trap focus, close on `Esc`, and return focus to the trigger on close.

#### Scenario: Modal opens from Home
- **WHEN** the visitor activates the Start streaming action
- **THEN** the modal opens with an empty title field, an empty description field, and Start / Cancel actions
...
On a valid submit the start flow SHALL send `POST /streams` with `{ title, description }` (description defaulting to `""`). On `201` it SHALL redirect to `/stream/{id}` using the returned `id`. On `400` it SHALL show a calm inline validation message and SHALL remain on the flow without redirecting.

#### Scenario: Start succeeds
- **WHEN** the submit returns `201` with an `id`
- **THEN** the portal navigates to `/stream/{id}` using that `id`

#### Scenario: Server rejects with 400
- **WHEN** the submit returns `400` (including a title the client did not pre-check)
...
The stream page (`/stream/{id}`) SHALL render calm placeholder content and an **End stream** action. Activating it SHALL send `DELETE /streams/{id}` and redirect to `/` on both `204` and `404`, without showing an error on `404`.
...
All calls to the §6 contract SHALL live in a single non-UI streams module; UI components SHALL NOT call `fetch` directly. The module SHALL parse responses from `unknown` and validate their shape before returning them (each stream has string `id` and `title`, and a present `description`). A malformed or non-JSON success body SHALL be treated as a failure, not rendered.
