...
- [ ] 1.1 `registerSubscriber`: aceptar `planCode`, validarlo contra los planes
      existentes y acumular el fallo en `errors[]` (ausente / código desconocido),
      no como un 4xx aparte
- [ ] 1.2 Crear la suscripción `ACTIVE` **dentro de la transacción existente** del
...
- [ ] 1.3 `POST /api/auth/register`: ampliar el esquema Zod compartido con el
      formulario; verificar que un fallo al crear la suscripción **deshace también la
      cuenta**
- [ ] 1.4 `app/(public)/registro/register-form.tsx`: selector de plan con los planes
      ...
      volver atrás
- [ ] 1.5 `/planes`: que las tarjetas "Empezar con {plan}" pasen el plan a `/registro`
- [ ] 1.6 Tests: alta con plan crea cuenta **y** suscripción activa; alta sin plan
      rechazada junto al resto de errores; plan inexistente rechazado; fallo a mitad
      de transacción no deja cuenta huérfana

## 2. Cambio de plan

- [ ] 2.1 Caso de uso `changePlan` en `src/use-cases/subscriptions/`: sin efecto si
...
- [ ] 2.2 El rechazo devuelve **cuántos sets debe devolver**, no un mensaje genérico;
      código de error propio, distinguible del límite de plan al solicitar
- [ ] 2.3 `PUT /api/subscriptions/me`: aceptar `planCode` además de `status`,
      resolviendo siempre la suscripción del usuario en sesión (design.md §4)
- [ ] 2.4 Registrar el cambio en `AuditLog` con el antes/después en `metadata` —
...
- [ ] 2.5 Tests: subir; bajar sin exceso; bajar con exceso rechazado (incluida la
...
- [ ] 3.1 `src/use-cases/rentals/request-set.ts`: eliminar la bifurcación y exigir
      suscripción activa; el rechazo distingue "sin plan activo" de "límite de plan"
- [ ] 3.2 Borrar `checkOneOffEligibility` (`src/domain/subscriptions/eligibility.ts`)
      y `computeOneOffPrice` (`src/domain/subscriptions/pricing.ts`)
- [ ] 3.3 Retirar los dos `SystemSetting` del puntual: `resolveSettings`, la semilla y
      los campos del formulario de `/backoffice/configuracion`
- [ ] 3.4 Tests: convertir los que ejercitaban la vía puntual
...
- [ ] 4.1 `prisma/seed.ts`: el usuario sin suscripción pasa a representar una "cuenta
      recién dada de alta"; revisar que los tests que dependían de él sigan probando
      lo que decían probar (Risks de design.md)
- [ ] 4.2 E2E: el recorrido de `e2e/circuito-completo.spec.ts` arranca ahora en un
      alta **con plan**; añadir el cambio de plan al recorrido del portal
- [ ] 4.3 Verificación completa: `tsc --noEmit`, `eslint .`, `vitest run`,
