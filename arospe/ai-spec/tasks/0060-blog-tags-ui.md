...
> [0025](0025-product-categories-ui.md) will expect.** There is no *"deletion is blocked with a
...
[0025's **R-3**](0025-product-categories-ui.md), and exactly the vulnerability class 0059's hand-off
...
- [ ] **Deleting a tag through the confirmation modal removes it in one click, with no intermediate
      ...
      [0025's highest-value test](0025-product-categories-ui.md#tests-to-perform), which proves a
...
- **D-1 — `save()` does not call `$this->validate()`, and the component does NOT compose
  ...
  modal open by construction" discipline [0025's **D-2**](0025-product-categories-ui.md) states.
...
- **D-2 — No hard-block delete. No usage count in any refusal, no blocked state, no error-bag key for
  ...
  *Rejected:* a confirmation modal that shows a post count "for information" once 0061 lands. Rejected
  as **D-5**, and separately because [0025's **D-3**](0025-product-categories-ui.md) records how easily
...
- **D-6 — `$tags` is `#[Locked]`, as is every id-carrying property.** This follows the *newer*
  ...
  [0025's **D-4**](0025-product-categories-ui.md), which deliberately leaves `$productCategories`
...
- **D-8 — Copy lives in a new `lang/{en,es}/blog-tags.php`; no shared `blog.php`.** Matches every
  ...
  *Rejected:* one shared `lang/{en,es}/blog.php` for tags, categories and posts. It recreates precisely
  the file-ownership hazard [0025's sequential-implementation note](0025-product-categories-ui.md)
...
- **F-1 — Story 0059 does not exist in code, so this entire interface contract is a claim about a task
  ...
  **F-2**](0025-product-categories-ui.md#findings) exactly. Recorded as a dependency, **not** a blocker
...
- **OQ-6 — Header summary line, and a search filter?** Two small UI questions with the same answer
  today.
  **(a) Neither (recommended)** — matching [0025's **D-7**](0025-product-categories-ui.md): nothing in
...
readings of [0059](0059-blog-tags-backend.md) and [0025](0025-product-categories-ui.md), with
