...
- **`price` is a string.** The `decimal:2` cast returns a **string** (0036 **D-7**, **R-7**,
  ...
  [0018](0018-sales-region-tax-configuration-ui.md) **D6** records for a `0.000` tax rate.
...
- **R-4 — `type="number"` sneaking into a weight or price field**, defeating **D-5** silently and
  ...
  [0018](0018-sales-region-tax-configuration-ui.md) records for its own rate field.
