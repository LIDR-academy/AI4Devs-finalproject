...
- **`/roles` is linked from the sidebar with no permission gating**, added after the view shipped and later nested inside a collapsible "Settings" group rather than sitting beside `/users` at the top level. [`resources/views/layouts/app/sidebar.blade.php`](../../resources/views/layouts/app/sidebar.blade.php) renders a static `flux:sidebar.item` to `route('roles.index')` for every authenticated user, inside a `flux:sidebar.group` (`expandable`, `:expanded="request()->routeIs('roles.*')"`) that groups store-configuration screens — "Roles & permissions" today, with more (e.g. billing details) expected to join it later. The grouping is purely presentational: access is still refused by `can:roles.manage` on the route and re-checked inside the component; permission-aware navigation is owned by story 0013, which is not yet built.
