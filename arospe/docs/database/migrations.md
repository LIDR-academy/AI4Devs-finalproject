...
_Last updated: 2026-09-06 — Story 0032 (Shipping geography catalog seed). Added `create_geography_entries_table` as the "no explicit FK index" rule's ninth confirming instance, and the first on this repo's first greenfield non-UUID (`bigint`) domain table — including the one place the migration deliberately diverges from its own task file's suggested `index('parent_id')`, correctly, per this page's existing rule. No other convention content changed this pass.

_Previously: 2026-09-05 — Collapsed the accumulating `_Previously:` footer chain (spanning tasks 0003–story 0029, ~5.3k characters) into this single line, per the doc-growth-management rule now codified in [contracts.md](../contracts.md#doc-growth-management-rule) and the docs-maintainer skill's Definition of Done. No convention content changed this pass.
