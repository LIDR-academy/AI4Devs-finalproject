...
_Last updated: 2026-09-07 — Story 0033 (Shipping zones — backend). Added the `shipping_zones` and `shipping_zone_geography_entry` tables — the admin-editable shipping zone catalog and its zone↔geography membership pivot ([PRD §2.4](../PRD/PRD.md#24-shipping)) — with their ER-diagram entries, full column/index documentation, why `ShippingZone` does not use `SoftDeletes`, why overlap between zones is allowed with no `UNIQUE` on `geography_entry_id` alone, why membership is literal rather than transitive, the inherit-the-connection-collation decision (closing 0032's own deferred **OQ-6** for these two tables), and the deliberately-not-yet-implemented in-use delete guard pre-shaped for story 0036. This is the first pivot in this schema crossing a UUID-keyed parent and this schema's one `bigint`-keyed parent (`geography_entries`) in the same composite primary key, and the tenth confirming instance of [migrations.md](migrations.md#an-fk-column-does-not-also-get-an-explicit-index-here)'s "no explicit FK index" rule. Updated the model count (ten → eleven) and the UUID-status Notes bullets accordingly._
