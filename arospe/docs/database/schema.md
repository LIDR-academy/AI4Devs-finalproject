...
_Last updated: 2026-09-07 — Story 0031a (product variant generator — the cartesian combination builder UI). Corrected the `GenerateProductVariantCombinations` subsection under [`product_variants`](#product_variants) again, folding forward the prior pass's own correction rather than stacking a second `_Previously:` block on it (doc-growth-management rule): the sentence saying the generator "remains backend only... until 0031a ships" is now false — 0031a has shipped, composing the generator UI onto 0031's already-shipped `App\Livewire\Products\VariantBuilder` (no second Livewire component, no new route), with `GenerateProductVariantCombinations` acquiring its first and only UI call site at `VariantBuilder::generateCombinations()`. No schema, column, index or query shape changed by this story — it consumes the action's existing signature and return shape unchanged. Content on every other table is unaffected.

_Previously: 2026-09-06 — Story 0032 (Shipping geography catalog seed). Added the `geography_entries` table — the read-only, three-level (country/comunidad autónoma/municipio) shipping geography catalog, physically independent of `sales_regions` (no shared table, no FK) — with its ER-diagram entry, full column/index documentation, and the seeder's chunked/idempotent/transactional shape. This is the first table in this schema keyed on a plain `bigint` rather than a UUID, the named exception [ADR 0001's Amendment 1](../decisions/0001-uuid-primary-keys.md#amendment-1-2026-08-27--the-scope-is-the-policy-not-the-list-of-seven) predicted in the abstract; see the new Amendment 5 there. Updated the model count (nine → ten) and the UUID-status Notes bullet accordingly._
