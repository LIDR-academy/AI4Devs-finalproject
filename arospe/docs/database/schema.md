...
Source: `database/migrations/2026_09_01_084836_create_product_categories_table.php` (story 0023) — the third Epic 2 domain table, after [`sales_regions`](#sales_regions) and [`media`](#media), and the first of the three that is a **standalone catalog with no relationships to anything else at all**: no FK in, no FK out. Per this file's own [ER-diagram rule](#er-diagram) — only a table that "carries a meaningful relationship" is diagrammed — `product_categories` earns **no entity in the mermaid diagram above**; it joins once `products.product_category_id` exists (story 0024).
...
- `app/Models/` holds five classes and they are three different kinds of thing: `User` (Epic 1's domain model), `SalesRegion` (task 0016 — the first Epic 2 domain model, and the first table this repo created greenfield with a UUID PK), `Media` (story 0019 — the second) and `ProductCategory` (story 0023 — the third, and the first with no relationships to anything at all), and `Role`, which is a `spatie/laravel-permission` subclass over the package's existing `roles` table rather than a new entity — it adds no column and no migration (see [architecture/authorization.md](../architecture/authorization.md#the-super-admin-roles-invariants)). This file grows a new section per table as the domain layer is built.
