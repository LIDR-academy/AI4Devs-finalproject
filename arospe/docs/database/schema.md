...
- **UUID (v7) primary keys ([ADR 0001](../decisions/0001-uuid-primary-keys.md)).** Status is split:
  ...
  - **`product_media` and `product_sales_region` are not counted against either list.** Both are pivot tables with no surrogate primary key of their own — composite `(product_id, media_id)` / `(product_id, sales_region_id)` over already-UUID FKs — the same shape as the vendored `spatie/laravel-permission` pivots (`role_has_permissions`, `model_has_roles`), which this ADR has never covered either. There is no "entity identifier" here for the ADR's policy to apply to.
