...
_Last updated: 2026-09-05 — Collapsed the accumulating `_Previously:` footer chain (spanning tasks 0004–story 0029b, ~22k characters) into this single line, per the doc-growth-management rule now codified in [contracts.md](../contracts.md#doc-growth-management-rule) and the docs-maintainer skill's Definition of Done. No schema, column, index or table content changed — this file already links out to [security/derived-column-invariants.md](../security/derived-column-invariants.md), `decisions/`, and [errors-log.md](../errors-log.md) for deep rationale rather than duplicating it inline, so a spot-check found nothing else worth trimming this pass.
