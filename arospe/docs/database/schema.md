...
_Last updated: 2026-07-22 — `users` UUIDv7 PK conversion is now implemented (Epic 1): updated the ER diagram, `users`/`passkeys` column tables, the polymorphic morph-key note (`model_uuid`), and the Notes to real current state; the six Epic 2/4 UUID entities remain future._
