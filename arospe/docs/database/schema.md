...
_Last updated: 2026-09-05 — Story 0029b (Product variant combination generator — backend). Backend
only, no migration, no new column, no index change. Widened [`product_variants`](#product_variants)
with a new subsection, [`GenerateProductVariantCombinations` is a second writer — through
`CreateProductVariant`, never in bulk](#generateproductvariantcombinations-is-a-second-writer--through-createproductvariant-never-in-bulk):
`App\Actions\Products\GenerateProductVariantCombinations` writes every generated variant and its pivot
rows exclusively through 0029's own `CreateProductVariant`, re-implementing nothing, inside one outer
`DB::transaction()` under which each `CreateProductVariant` call becomes a savepoint — a per-combination
refusal rolls back only that savepoint, not the batch. Its one new read (a duplicate-combination
pre-check) is one query for the whole batch, served as a covering scan by the table's own
`unique(product_id, combination_hash)`. **This story adds no route, Livewire component, Blade view,
permission or `config/modules.php` entry** — see [architecture/authorization.md](../architecture/authorization.md#product-variant-actions-gate-against-the-parent-product-not-a-new-policy)
for why the generator gates `update` on the parent product once, up front, rather than per row, and
[security/derived-column-invariants.md](../security/derived-column-invariants.md#second-confirming-instance-a-generators-own-outer-transaction--attempts-fires-only-at-nesting-level-1)
for the corrected claim about `Editor::save()` being this domain's only outer transaction.

_Previously: 2026-09-05 — Story 0029a (Attribute type & value in-use delete guards — backend). Backend-only, no migration, no new column and no policy change — this story adds the *message* in front of two FKs [`product_variants`](#product_variants)/[`product_variant_values`](#product_variant_values) already ship. Widened [`product_attribute_types`](#product_attribute_types) with a ✅ block for `App\Models\ProductAttributeType::variantUsageCount()` (`COUNT(DISTINCT pvv.product_variant_id)`, the single source of the count both `App\Actions\Products\DeleteProductAttributeType` and `App\Livewire\Products\AttributeTypes\Index::confirmDelete()` consume) and the gate-then-count-then-1451-backstop shape `DeleteProductAttributeType` now follows, inherited wholesale from `product_categories`' own in-use block (0024b's D-14) rather than re-derived. Widened [`product_attribute_values`](#product_attribute_values) with a new subsection recording that `SyncProductAttributeValues`' delete branch — the × next to a value inside the type's edit modal, the *everyday* path rather than the type-delete modal — previously had **no catch at all** around its `DELETE`, so a raw `1451` surfaced as an unhandled `QueryException` (a 500); it now refuses per-value, before any row is removed, with a catch narrowed to MySQL error 1451 via `errorInfo[1]` in its **own** try/catch, deliberately never folded into `writeRow()`'s pre-existing `23000` catch (which means "duplicate value" and would mislabel the refusal). **Verified as unchanged rather than assumed:** every other table section, including both of story 0029's own tables — this story alters no column, index or FK anywhere in this schema.
