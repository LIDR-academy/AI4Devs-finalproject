...
_Last updated: 2026-09-06 — Story 0030a (attribute value rename — usage warning and SKU-collision error rendering). Added `App\Models\ProductAttributeValue::variantUsageCounts()` — a bulk, keyed-by-value-id count over `product_variant_values`, backing the attribute types screen's per-row rename-usage notice — as a new subsection under [`product_attribute_values`](#product_attribute_values) cross-referencing it against the two existing per-value/per-type query shapes (`firstValueInUse()`, `variantUsageCount()`) so the three stay distinguished rather than silently parallel. No schema, column, index or table content changed. No new `_Previously:` chain added, per the doc-growth-management rule in [contracts.md](../contracts.md#doc-growth-management-rule).
