...
| Ability | Signature | Rule | Authorized from |
| --- | --- | --- | --- |
| `viewAny` | `(User $actor)` | holds `shipping.view` | nothing yet — no route, no Livewire component (D-8) |
| `create` | `(User $actor)` | holds `shipping.create` | `App\Actions\Shipping\CreateShippingZone::__invoke()`, its own first statement |
| `update` | `(User $actor, ShippingZone $target)` | holds `shipping.edit` — `$target` is **not** consulted | `App\Actions\Shipping\RenameShippingZone::__invoke()` and `App\Actions\Shipping\SyncShippingZoneGeography::__invoke()`, each its own first statement |
| `delete` | `(User $actor, ShippingZone $target)` | holds `shipping.delete` — `$target` is **not** consulted | `App\Actions\Shipping\DeleteShippingZone::__invoke()`, its own first statement |
...
**D-8 is unaffected by that correction, and the two decisions must not be conflated.** This story still ships **no route and no Livewire component** — that half of D-8's reasoning (the URI and view path are already taken by 0035; the zone editor's core interaction is blocked on 0022/0034's unbuilt geography picker) is untouched. What changed is only *how much* is riding on the still-open Definition-of-Done hand-off to the consuming UI story: before Phase 4's fix, that hand-off carried the *entire* authorization perimeter; after it, `ShippingZonePolicy` already has real, enforced call sites, and the hand-off is narrower — defence-in-depth for an already-real gate, plus the separate, un-touched-by-this-fix concern of keeping the zone id feeding `Rule::unique()->ignore()` server-authoritative via `#[Locked]` plus a re-read (see [security/livewire-authorization.md](../security/livewire-authorization.md#locked-is-what-makes-ruleunique-ignore-safe-here)).
