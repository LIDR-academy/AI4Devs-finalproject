...
_Last updated: 2026-09-06 — Story 0031 (Product variants — the variant builder inside the product editor, UI). **No new permission, role, middleware alias or policy — still seven, and still no `ProductVariantPolicy`.** Corrected, in place with what it used to say quoted, [Product variant actions gate against the parent product, not a new policy](#product-variant-actions-gate-against-the-parent-product-not-a-new-policy)'s claim that the actions' own gate was "the only enforcement layer that exists" — `App\Livewire\Products\VariantBuilder` is now the predicted second layer, re-authorizing `update` on the parent `Product` (via a bare `Gate::authorize()`, not `LogRefusedPrivilegedAttempt` — the one deliberate divergence from `Products\Index`'s/`Editor`'s shape) as the first statement of ten mutating/disclosing methods, giving `ProductPolicy` real call sites on a third screen. **No new `docs/errors-log.md` entry from this story's own authorization surface** — see [errors-log.md](../errors-log.md) for this story's one new entry, about a browser-test detector rather than an authorization finding. ⚠️ **Flagged, not fixed, this pass**: this file is ~273k characters with 25 accumulated `_Previously:` blocks, well past the ~150k doc-growth-management budget (see [contracts.md](../contracts.md#doc-growth-management-rule)) — a split/collapse is due but out of scope for this sync pass.

_Previously: 2026-09-05 — Story 0029b (Product variant combination generator — backend). **No new permission, role, middleware alias or policy — still seven, and still no `ProductVariantPolicy`.** Widened [Product variant actions gate against the parent product, not a new policy](#product-variant-actions-gate-against-the-parent-product-not-a-new-policy) with the generator's own paragraph: `App\Actions\Products\GenerateProductVariantCombinations` self-authorizes `update` on the parent `Product` via the same `ProductPolicy`, the same shape its three siblings already use, with the one real difference being **when** it asks — once, up front, before validation/the value-set read/the cap check/its own transaction, rather than once per generated row — a cardinality difference driven by the authorization question being identical for every combination in the batch, not a new rule. `CreateProductVariant`'s own per-row gate still runs on every generated row and is not redundant; there is deliberately no "skip the gate" parameter to bypass it. This story ships **no Livewire component and no route** (story 0031's), so the generator's own gate plus `CreateProductVariant`'s per-row gate remain the only enforcement layers, exactly as story 0029 left them. **No new `docs/errors-log.md` entry** — this story's one genuinely new security fact (nested `attempts:` firing only at the outermost transaction, confirmed on a second, independent call path) is a mechanical rule recorded on [security/derived-column-invariants.md](../security/derived-column-invariants.md#second-confirming-instance-a-generators-own-outer-transaction--attempts-fires-only-at-nesting-level-1), matching this project's established precedent.

_Previously: 2026-09-04 — Story 0029 (Product variants — core backend). **No new permission, role, middleware alias or policy — still seven policies, and this story is the first to ship real domain actions with `Gate::authorize()`-shaped calls that deliberately do NOT add an eighth one.** Added [Product variant actions gate against the parent product, not a new policy](#product-variant-actions-gate-against-the-parent-product-not-a-new-policy): all three variant actions (`CreateProductVariant`, `UpdateProductVariant`, `DeleteProductVariant`) self-authorize `update` on the variant's **parent `Product`** through `ProductPolicy`, never a `ProductVariantPolicy` — a variant is a product sub-resource with no per-row distinction from a sibling variant, the same reasoning `ProductCategory`/`ProductAttributeType` already establish. Records why `UpdateProductVariant`/`DeleteProductVariant` re-fetch the whole variant row before authorizing rather than merely `load('product')`ing the caller-supplied instance (Phase 4 finding F-8 — a relation reload re-reads the product but resolves *which* product from a caller-controlled in-memory foreign key), cross-referenced to the new [security/derived-column-invariants.md](../security/derived-column-invariants.md) page. **The section also records a real, shipped Three Amigos plan reversal**: the task file originally planned no self-authorization inside the actions at all, citing a stale claim about `CreateUser`/`UpdateUser`'s own shape that task 0008a had already falsified (it moved authorization *into* those actions specifically so a non-dashboard caller would inherit it) — Phase 2 review caught the stale citation and reversed the plan, so all three actions now follow the project's fully-established action-owns-the-rule convention rather than reopening a gap this page already closed once. Story 0029 ships **no Livewire component and no route** (story 0031's), so the actions' own gate is currently the *only* enforcement layer — there is no second, component-level re-check yet to call a redundancy, and 0031 adding one will be a genuine second layer, not a duplicate. **No change to the permission catalog, the seeded roles, or `RolePermissionSeeder`** — variant operations are covered by the existing `products.*` grant the catalog already documents as spanning "products, product categories and variants." **No new `docs/errors-log.md` entry** — this story's Phase 4 findings (the F-8 relation-reload gap above, plus the derived-column-invariant findings) are mechanical security rules with their own full write-up on the new [security/derived-column-invariants.md](../security/derived-column-invariants.md) page, matching this project's established precedent for not duplicating audit findings into this log.

_Previously: 2026-09-04 — Story 0027 (Products list + editor UI). **No new permission, role or middleware alias; still six policies** — this story gives `ProductPolicy` the call sites its five predecessors left open, rather than adding a seventh policy. Rewrote [`ProductPolicy`](#productpolicy--the-sixth-policy-and-the-second-built-entirely-for-a-screen-that-does-not-exist-yet)'s section around the shipped screens: `viewAny` finally has a caller (`App\Livewire\Products\Index::mount()`), and `create`/`update`/`delete` are now gated at **two** layers — the 0024 actions' own self-authorization, plus `Products\Index`/`Products\Editor` re-checking the identical abilities through `LogRefusedPrivilegedAttempt->authorize()` — the same shape `ProductCategoryPolicy` already established for its sibling module. **One stale cross-reference corrected in place**: `ProductCategoryPolicy`'s own section used to contrast its four-of-four shape against `ProductPolicy`'s "viewAny still waits for story 0027"; that wait is over, so the sentence is rewritten with what it used to say quoted, per this project's audit-authored-page convention. Widened [the sidebar registry section](#the-second-half-of-a-module-gate-the-sidebar-registry) with a ✅ for `items.products`, the **fifth** entry in a row that costs only two array literals plus two lang leaves, with neither `sidebar-nav.blade.php` nor the layout touched. Corrected the stale **Where it lives** "three gated routes" row, which had not been updated since task 0017 despite two more gated routes (`product-categories.index`, and now three `products.*` routes) shipping since — now names all six. **No new `docs/errors-log.md` entry** — this story's own security-audit findings (F-1 through F-6, N-1) are mechanical rules about a specific screen's methods, already recorded in the task file's own Phase 4/5 records and in [security/livewire-authorization.md](../security/livewire-authorization.md) and [security/array-validation-bounds.md](../security/array-validation-bounds.md), matching this project's established precedent for not duplicating audit findings into this page.

_Previously: 2026-09-03 — Story 0026 (Product ↔ Sales Region assignment and tax resolution backend). **No new permission, role, policy or route** — this story ships zero authorization surface by design (D8): a new `product_sales_region` pivot, a resolver and an options resolver, with no `Gate::authorize()` call anywhere in its four new classes. Widened [`ProductPolicy`](#productpolicy--the-sixth-policy-and-the-second-built-entirely-for-a-screen-that-does-not-exist-yet)'s notes from five to six with a new bullet: `App\Actions\Products\SyncProductSalesRegions` is this codebase's **fourth** shipped instance of "a collaborator invoked only by an already-authorized action needs no gate of its own" (after `GenerateImageConversions`, `EnforceGrantorPermissionScope` and `SyncProductGallery`), matching `SyncProductGallery`'s shape rather than the three self-authorizing product actions', enforced by a reachability test in the story's own Feature suite; its two sibling new classes (`ResolveProductTaxRate`, `SearchSalesRegions`) self-authorize nothing for two further, distinct reasons each states in its own docblock (a pure read visible to any `products.view`/`sales-regions.view` holder; catalog data treated as uniformly visible to any authenticated admin). See [database/schema.md](../database/schema.md#product_sales_region) for the pivot itself and [conventions/base-standards.md](../conventions/base-standards.md#directory-structure) for the fuller reasoning. **Verified as unchanged rather than assumed:** the module-gate pattern, the sidebar registry, step-up authentication, the refusal-recording recipe, and every other policy on this page.
