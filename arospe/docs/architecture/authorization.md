...
- **`hasPermissionTo()` in the body, `public const` for the names, no `$target` branch** — all three inherited unchanged from `RolePolicy`/`SalesRegionPolicy` rather than re-decided. `VIEW_PERMISSION` / `CREATE_PERMISSION` / `EDIT_PERMISSION` / `DELETE_PERMISSION` follow [naming.md](../conventions/naming.md#permission-names)'s "name a permission once on the class that owns the rule", which now makes `UserPolicy`'s repeated literals the sole outlier among six policies — `App\Policies\ProductCategoryPolicy` (story 0023, no route or component calling it yet — see [database/schema.md](../database/schema.md#product_categories)) and `App\Policies\ProductPolicy` (story 0024, three of four abilities with a real call site — see [`ProductPolicy`](#productpolicy--the-sixth-policy-and-the-second-built-entirely-for-a-screen-that-does-not-exist-yet) below) are the fourth and fifth followers. `tests/Feature/Policies/MediaPolicyTest.php` asserts a Super Admin passes every ability while holding **zero** permission rows, via `Gate::before`.
...
[`App\Policies\ProductPolicy`](../../app/Policies/ProductPolicy.php) (story 0024) gates the same `products.*` permissions [`ProductCategoryPolicy`](../database/schema.md#product_categories) (story 0023) already uses — a product category is a product sub-resource, so it shares the module rather than getting its own — but it is a genuinely different shape from its sibling: three of its four abilities have **real call sites today**, in this story's own actions, rather than waiting for story 0025's/0027's not-yet-built UI.
...
- **Three of four abilities have a real call site; `viewAny` does not — and this is `MediaPolicy`'s four-abilities-two-used shape, not `ProductCategoryPolicy`'s zero-call-sites gap.** `ProductCategoryPolicy` was created with **no** caller at all, because story 0023 ships no route and no Livewire component — a deliberate hand-off recorded as a ⚠️ on [schema.md](../database/schema.md#product_categories). `ProductPolicy` is different: `create`/`update`/`delete` are wired in from day one, because this story's actions self-authorize (see the next bullet), and only `viewAny` waits for a consumer — the identical "define an ability when you can name what will ask it" reasoning [`MediaPolicy`](#mediapolicy--the-fourth-policy-and-the-first-behind-no-route-at-all) already establishes for its own `update`/`delete` split. Do not read the shared zero-caller *ability* as evidence of a shared *pattern* between the two policies — the two are opposite shapes that happen to overlap on one row.
