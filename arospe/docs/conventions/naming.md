...
_Last updated: 2026-09-08 — Story 0080 (sidebar navigation grouping and nesting), Phase 6 docs sync. Corrected the registry-mirroring rule's foundational example (task 0013's original `groups.platform`/`groups.settings`/`groups.taxes` + four-item shape), which had gone stale: `groups.platform`/`groups.taxes` are retired by story 0080's registry restructuring, and the registry gained a third mirrored array, `clusters` (`clusters.products`, `clusters.store_settings`), alongside the now-current `groups.store`/`groups.settings` and seven `items.*` leaves — verified against the real, shipped `lang/en/navigation.php`. The registry-mirroring rule itself is unchanged; only its worked example needed updating. Story 0080 has since closed (Phase 7) and moved to `ai-spec/tasks/done/`. No new `<Noun>ValidationRules` trait, route name, permission, `Index`-in-a-subfolder instance or boolean-property shape (this story adds no PHP class of any kind — a frontend-only `config`/Blade/lang restructuring).

_Previously: 2026-09-06 — Story 0031 (Product variants — the variant builder inside the product editor, UI). Added `App\Livewire\Products\VariantBuilder` → `resources/views/livewire/products/variant-builder.blade.php` as the **fourth confirmation** of the ordinary (non-`Index`-exception) mirror rule, alongside `Media\Gallery`/`WysiwygEditor`/`SearchableMultiSelect` — the naming rule is unchanged; what's new is that this is the app's first **nested child** component embedded inside another module's own routed page rather than mounted at a route or a modal of its own, and the mirror rule does not care. **This story adds no new class/file table row** (a Livewire component, governed by the mirror-rule section above, not the table), **no new route name, permission, translation-key group or boolean property beyond the existing `show`-prefix pattern** (`$showForm`/`$showGallery`/`$showDeleteModal`). **Verified as unchanged rather than assumed:** every other section on this page, including the `<Noun>ValidationRules` traits section — this story composes the existing seventh trait (`ProductVariantValidationRules`) unchanged rather than adding an eighth.

_Previously: 2026-09-05 — Story 0029b (Product variant combination generator — backend). Added one class/file table row (`App\Actions\Products\GenerateProductVariantCombinations`) — an imperative-verb-phrase action name, nothing new to decide. Appended a paragraph recording the **two new methods** (`variantAttributeTypeIdsRules()`, `variantAttributeTypeIdRules()`) added to 0029's existing seventh `App\Concerns\ProductVariantValidationRules` trait — still no eighth trait, and a second, generator-scoped trait was considered and rejected for the identical collision/flat-single-concern reasons the rest of this section already gives. **This story adds no new route name, permission, translation-key group, boolean property or `Index`-in-a-subfolder instance.** **Verified as unchanged rather than assumed:** every other section on this page.
