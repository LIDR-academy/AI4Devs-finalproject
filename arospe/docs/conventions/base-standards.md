...
app/
  ...
  Livewire/            Livewire components, grouped by area (Users/, Roles/, SalesRegions/,
                       Media/, ProductCategories/, Products/AttributeTypes/, Components/, Settings/,
                       Settings/TwoFactor/, Actions/, plus Dev/ — see below). Components/ (story 0021, extended by 0022) is not a module area
...
routes/                 web.php, plus one file per functional area that web.php requires
                        ...
                        product-categories.php, product-attribute-types.php) — no api.php yet.
                        web.php also holds ONE environment-gated dev route (story 0020's browser-
...
tests/
  Feature/              Feature tests, mirrors app structure (Actions/Auth/, Auth/, Settings/,
                        ...
                        Products/, Components/, Dev/, Models/, Policies/, Authorization/,
                        Navigation/, ...)
  ...
  Browser/              Pest browser tests. Mirrors app structure (Auth/, Media/, Components/) —
                        but three of the eight files sit flat instead (UsersIndexTest.php,
                        RolesIndexTest.php, SalesRegionsIndexTest.php; the previous "two of six"
                        count here missed RolesIndexTest.php entirely — corrected by story 0022); see
...
`app/Livewire/Dev/` (story 0020) is the one subfolder here that is **not** a functional area and is **not permanent**, and both facts have to be visible from the folder rather than inferred. It holds `MediaGalleryHarness`, a throwaway host page whose only purpose is to give `App\Livewire\Media\Gallery` — a modal embed with no route of its own — a URL a browser test can `visit()`. Four rules come with a folder like this, and they are what separate scaffolding from surface:

- **The route is gated at *registration* time, not by middleware.** `if (app()->environment('testing', 'local'))` in `routes/web.php`, so the URL does not exist in production rather than existing and refusing. A `can:`/`permission:` middleware would leave it registered and discoverable.
- **It carries `auth` + `verified` anyway**, and a second `abort_unless(app()->environment(…), 404)` in `mount()` — defence in depth behind the gate, covering the one case the gate cannot (a `route:cache` built under the wrong environment is replayed verbatim and never re-runs the `if`).
- **A test asserts its absence from the route *collection*, not a 404**, because the control is non-existence rather than refusal and only the collection assertion distinguishes them ([`tests/Feature/Dev/MediaGalleryHarnessRouteTest.php`](../../tests/Feature/Dev/MediaGalleryHarnessRouteTest.php)).
- **It names its own deletion trigger, in every file it occupies** — the component, its view, and the `routes/web.php` block each carry a comment saying story 0027 deletes it once the product editor supplies a real host page. Scaffolding without a named expiry is just surface nobody has admitted to yet; the same commitment is recorded in [api/routes.md](../api/routes.md#devmedia-gallery-harness--temporary-environment-gated-scaffolding) so it is visible from the docs a reviewer actually opens.

`routes/` follows the same one-per-area shape: `web.php` declares only the app-wide routes (`home`, `dashboard`) and then `require`s one file per functional area — `settings.php`, `roles.php`, and `users.php` since task 0040, which moved `users.index` out of `web.php` so it stops being the one route that didn't follow the pattern, plus `sales-regions.php` since task 0017, `product-categories.php` since story 0025 and `product-attribute-types.php` since story 0028. A new area's routes go in a new `routes/<area>.php` with its own middleware group, appended as another `require` line rather than inlined into `web.php`; what each route contract actually is belongs to [api/routes.md](../api/routes.md).
...
> **Seven live examples: `User` (Epic 1), `SalesRegion` (task 0016), `Media` (story 0019), `ProductCategory` (story 0023), `Product` (story 0024), and `ProductAttributeType` + `ProductAttributeValue` (story 0028).** All seven are real UUID (v7) PK models. `User` got there by conversion, per [ADR 0001 — UUID primary keys](../decisions/0001-uuid-primary-keys.md); `SalesRegion` was the first model in this repo created that way from day one, and remains the one to copy for a plain greenfield table. **`sales_regions`, `media`, and `product_attribute_types`/`product_attribute_values` are not among the ADR's original seven entities** — each shipped under a confirmed project-wide policy (UUID v7 for every new Epic 2 business entity, with a high-volume geography lookup table excepted and left `bigint`) recorded in [ADR 0001's Amendment 1](../decisions/0001-uuid-primary-keys.md#amendment-1-2026-08-27--the-scope-is-the-policy-not-the-list-of-seven) — landed with story 0019, and story 0028's pair is the policy's third and fourth confirmed instance rather than needing its own amendment, per the task file's own D9. **`product_categories` and `products` are the opposite case**: both are literally among the ADR's original seven named entities, so neither needed an amendment at all — see [ADR 0001's Amendment 2](../decisions/0001-uuid-primary-keys.md#amendment-2-2026-09-01--product-categories-lands-inside-the-original-seven) and [Amendment 3](../decisions/0001-uuid-primary-keys.md#amendment-3-2026-09-01--products-is-the-second-of-the-original-seven-to-ship). `product_media`, story 0024's gallery pivot, and `product_sales_region`, story 0026's, both have **no UUID PK of their own at all** — composite keys over already-UUID FKs, the same shape the vendored permission pivots use — so both fall outside this convention entirely rather than counting as additional examples. Four of the ADR's originally-named six not-yet-implemented entities (product variants, blog categories, blog tags, blog posts) still do not exist in code. Read the ADR for rationale and [database/schema.md's Notes](../database/schema.md#notes) for what is actually keyed this way. This subsection is only the code-shape convention.
...
_Last updated: 2026-09-03 — Story 0028 (Product variant attribute types & values — backend). **Directory listing:** added four classes to the `Actions/Products/` line (`CreateProductAttributeType`, `UpdateProductAttributeType`, `DeleteProductAttributeType`, `SyncProductAttributeValues`), `ProductAttributeTypePolicy` to `app/Policies/`, `Products/AttributeTypes/` to the `Livewire/` grouping, and `product-attribute-types.php` to both the top-level `routes/` line and the one-per-area convention paragraph. Added a new paragraph recording `SyncProductAttributeValues` as this codebase's **fifth** shipped instance of "a collaborator invoked only by an already-authorized action needs no gate of its own" (after `GenerateImageConversions`, `EnforceGrantorPermissionScope`, `SyncProductGallery` and `SyncProductSalesRegions`) — the first instance of the pattern owning a diff over a plain child table rather than a pivot, and why that diff is what makes the id-stability guarantee story 0029 depends on hold. **Rewrote the UUID-primary-keys blockquote**: seven live examples now (`ProductAttributeType`/`ProductAttributeValue` the sixth and seventh), both falling under [ADR 0001 Amendment 1](../decisions/0001-uuid-primary-keys.md#amendment-1-2026-08-27--the-scope-is-the-policy-not-the-list-of-seven)'s general policy as its third and fourth confirmed instance (no further ADR amendment needed, per the story's own D9), and corrected the same blockquote to also name `product_sales_region` (story 0026) among the pivot tables falling outside this convention entirely — a gap this page had carried unnoticed since story 0026 shipped. This story adds **no** new permission, seeder change or `config/modules.php` entry — see [database/schema.md](../database/schema.md#product_attribute_values) for the new tables and [api/routes.md](../api/routes.md#product-attribute-typesindex--the-fifth-permission-gated-route-and-the-first-shipped-with-a-backend-only-placeholder-view-since-task-0017) for the new gated route, including why its sidebar entry is not yet shipped. **Verified as unchanged rather than assumed:** the stack-versions table, the model conventions (both new models follow the `#[Fillable]` convention trivially, and `#[Fillable]`'s existing omission-as-guard reasoning applies unchanged — neither table's mass-assignable columns include a server-derived value), the app-owned-config-file section (`config/modules.php` untouched — no sidebar entry shipped in this story, per Q6/D-Q6's placeholder-view scope), the Livewire class-based convention (`AttributeTypes\Index` follows it unmodified, including the `Index`-in-a-subfolder exception), and the quality gates.
