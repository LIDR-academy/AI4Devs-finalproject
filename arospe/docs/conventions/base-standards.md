...
app/
  ...
  Livewire/            Livewire components, grouped by area (Users/, Roles/, SalesRegions/,
                       Media/, ProductCategories/, Components/, Settings/, Settings/TwoFactor/,
                       Actions/, plus Dev/ — see below). Components/ (story 0021, extended by 0022) is not a module area
...
routes/                 web.php, plus one file per functional area that web.php requires
                        ...
                        product-categories.php) — no api.php yet.
...
`routes/` follows the same one-per-area shape: `web.php` declares only the app-wide routes (`home`, `dashboard`) and then `require`s one file per functional area — `settings.php`, `roles.php`, and `users.php` since task 0040, which moved `users.index` out of `web.php` so it stops being the one route that didn't follow the pattern, plus `sales-regions.php` since task 0017 and `product-categories.php` since story 0025. A new area's routes go in a new `routes/<area>.php` with its own middleware group, appended as another `require` line rather than inlined into `web.php`; what each route contract actually is belongs to [api/routes.md](../api/routes.md).
