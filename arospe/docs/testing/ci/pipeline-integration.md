...
_Last updated: 2026-08-26 — CI database connection gap (`ai-spec/tasks/ci-database-connection-gap.md`): added the cross-reference from **Database provisioning** to [commands.md's Database prerequisite section](commands.md#database-prerequisite), which documents what a contributor must provision locally to satisfy the same `phpunit.xml` pin this section describes for CI. The **Database provisioning** paragraph itself was added in the same commit as the fix (`55ba248`) and needed no further correction — verified live against a real MySQL connection (866/866 tests passing) before this pass. Closes the doc-pass item this task's checklist left open._
