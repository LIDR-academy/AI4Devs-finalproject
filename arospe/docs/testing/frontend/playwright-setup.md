...
tests/
  ...
  Browser/     Real-browser end-to-end tests (Pest browser plugin), RefreshDatabase applied too;
               ...
               tests/Browser/Media/GalleryTest.php (the media gallery modal, story 0020) and
               tests/Browser/Components/WysiwygEditorTest.php +
               tests/Browser/Components/WysiwygEditorOutputHtmlTest.php (the WYSIWYG editor, story 0021)
...
> ✅ **The mirrored-subfolder rule has now held twice running, and the two flat files are a minority.** This page has said since task 0006 that the flat `UsersIndexTest.php` is debt and *"the next browser test goes in its mirrored subfolder"*; task 0018 shipped flat anyway, which is why the lesson was re-aimed at Phase 2 — **a story file that names a test path is making a convention decision, so the path belongs in the review, not in the implementation.** Story 0020 is where that first landed (`tests/Browser/Media/GalleryTest.php`, named explicitly in its task file), and story 0021 repeats it: both `tests/Browser/Components/WysiwygEditorTest.php` and `tests/Browser/Components/WysiwygEditorOutputHtmlTest.php` sit under the mirrored `Components/` subfolder, matching `App\Livewire\Components\WysiwygEditor`'s own `tests/Feature/Components/` counterpart, with no departure to record. Two of six files (`UsersIndexTest.php`, `SalesRegionsIndexTest.php`) are still flat; that remains debt, and moving them is nobody's story yet — but the flat form is no longer the pattern a new test author would infer from counting files.

The suite is wired up (task 0006b). All four pieces are real and verifiable right now:

- **`tests/Browser/` exists**, holding `Auth/LoginSmokeTest.php` (plus `UsersIndexTest.php` since task 0006, `SalesRegionsIndexTest.php` since task 0018, `Media/GalleryTest.php` since story 0020, and `Components/WysiwygEditorTest.php` + `Components/WysiwygEditorOutputHtmlTest.php` since story 0021) — a deliberately assertion-light canary that visits `/login`, asserts its user-visible text renders, and calls `assertNoJavaScriptErrors()`. Its job is proving the pipeline runs end to end, **not** covering sign-in behavior (that belongs to `tests/Feature/Auth/AuthenticationTest.php` and to whichever story owns sign-in browser coverage). Don't grow product assertions into it.
...
Mirror the app structure inside `tests/Browser/` (e.g. `tests/Browser/Auth/`, `tests/Browser/Settings/`) exactly as `tests/Feature/` already does — `Auth/LoginSmokeTest.php` establishes that. **Two of six files depart from it, and neither is the most recent one.** Task 0006 shipped `tests/Browser/UsersIndexTest.php` **flat**, where the mirror would put it at `tests/Browser/Users/IndexTest.php` (its component-level counterpart *is* at `tests/Feature/Users/IndexRenderingTest.php`), and this file recorded it as "the real current state, not a second convention — put the next browser test in its mirrored subfolder". The next browser test was task 0018's `tests/Browser/SalesRegionsIndexTest.php`, and it shipped **flat too** — its path likewise came from a story file, written before anyone opened this page. **That is where the drift stopped.** Story 0020's `tests/Browser/Media/GalleryTest.php` and story 0021's `tests/Browser/Components/WysiwygEditorTest.php` / `Components/WysiwygEditorOutputHtmlTest.php` all shipped in their mirrored subfolders, each because the story's own task file named the path explicitly and cited this section as the reason — so the mirrored form is now the majority (four of six) rather than the minority it was when this paragraph last said the opposite. **The mirrored subfolder is still, and has always been, the convention** (`tests/Browser/SalesRegions/IndexTest.php` is where a fifth Sales Regions browser file belongs), and the two flat files are debt, not precedent — but a reader counting files today would no longer mistake flat for the pattern. The practical lesson holds regardless of the count: **a story file that names a test path is making a convention decision, so the path belongs in the Phase 2 review** — the two stories that got this right did so for exactly that reason. Note the artisan-first workflow used everywhere else needs one manual step here: `php artisan make:test --pest LoginBrowserTest` still places the file under `tests/Feature/`, so move it into `tests/Browser/` after generating it.
...
⚠️ **And a residual can survive all of that, which is worth stating so the next author stops sooner.** `tests/Browser/Media/GalleryTest.php`'s reopen test is this repo's second honestly-recorded flaky browser test, alongside `SalesRegionsIndexTest.php`'s *"measurably reduces but does not provably eliminate"*. Its docblock records three fix attempts, each backed by execution rather than reasoning: `assertVisible()` (mechanically the correct primitive — still observed to fail at that line in one isolated run), a direct non-chained `Execution::instance()->wait(0.5)` to dodge the `__call()` routing entirely (no better), and `Playwright::usingTimeout(15000, ...)` around the `fill()` (**worse**). The evidence points at a real, occasional delay in the click → Livewire round trip → Alpine `fluxModal()` → native `<dialog>.showModal()` chain exceeding the 5000 ms ceiling — external timing variance, not something the test's own code can wait its way around. **Everything in the wait/assertion-permutation category has now been tried; the next lever is a Pest-level retry on that one test, not a fourth permutation.**
