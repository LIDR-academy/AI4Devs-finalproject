...
interface AppointmentCardProps {
  appointment: {
    ...
    status: "pending" | "confirmed" | "cancelled" | "rejected" | "approved"
    type: string
  }
...
const AppointmentCard: React.FC<AppointmentCardProps> = ({ appointment, onApprove, onReject }) => {
  ...
  useEffect(() => {
    try {
      ...
      setFormattedDate(format(dateObj, "PPPP", { locale }))
      setFormattedTime(format(dateObj, "p", { locale }))
    } catch {
      setFormattedDate(appointment.date)
      setFormattedTime(appointment.time)
    }
  }, [appointment.date, appointment.time, i18n.language])
  ...
  // Mostrar badge solo si está pendiente
  const showBadge = appointment.status === "pending"
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow duration-200">
      ...
      <div className="flex items-start justify-between mb-4">
              ...
              {appointment.patient.phone && (
                <span className="text-xs text-gray-600 flex items-center gap-1">
                  <svg className="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H5a2 2 0 01-2-2V5zm0 14a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H5a2 2 0 01-2-2v-2zm14-14a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zm0 14a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                  </svg>
                  {appointment.patient.phone}
                </span>
              )}
        ...
        {showBadge && (
          <span className="px-3 py-1 rounded-full text-xs font-medium text-yellow-600 bg-yellow-50">
            {t("medicalAgenda.status.pending")}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center space-x-4 mb-6">
        <div className="flex items-center space-x-2 text-sm text-gray-600">
          ...
          <span>
            {t("appointment.date")}: {formattedDate}
          </span>
        </div>
        <div className="flex items-center space-x-2 text-sm text-gray-600">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span>
            {t("appointment.time")}: {formattedTime}
          </span>
        </div>
      </div>
          ...
          >
            {isRejecting ? (
              <>
                ...
                <span>{t("medicalAgenda.appointmentCard.rejecting")}</span>
              </>
            ) : (
              <>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span>{t("medicalAgenda.appointmentCard.rejectButton")}</span>
              </>
            )}
          </button>
          ...
          >
            {isApproving ? (
              <>
                ...
                <span>{t("medicalAgenda.appointmentCard.approving")}</span>
              </>
            ) : (
              <>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span>{t("medicalAgenda.appointmentCard.approveButton")}</span>
              </>
            )}
          </button>
        </div>
      ...
      {/* Confirmed/Cancelled/Rejected/Approved state message */}
      {appointment.status !== "pending" && (
        <div
        ...
        >
          {appointment.status === "confirmed" || appointment.status === "approved"
            ? t("medicalAgenda.appointmentCard.approved")
            : t("medicalAgenda.appointmentCard.rejected")}
        </div>
      )}
    </div>
  )
