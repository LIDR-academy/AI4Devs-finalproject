...
export async function resetOperationalDashboard(input: {
  mode: OperationalResetMode;
  closeActiveSessionAs: CloseActiveSessionAs;
}): Promise<OperationalResetResult> {
  const response = await fetch(`${backendUrl}/dashboard/operational/reset`, {
    method: "POST",
  ...
  return response.json() as Promise<OperationalResetResult>;
}
