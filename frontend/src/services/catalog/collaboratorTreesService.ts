...
export type CollaboratorTreeSort = 'modificado_en,desc' | 'modificado_en,asc' | 'creado_en,desc' | 'creado_en,asc'
...
const COLLABORATOR_TREES_PATH = '/api/catalog/trees'
...
export async function fetchCollaboratorTrees(
  ...
  return apiFetch<CollaboratorTreePageResponse>(COLLABORATOR_TREES_PATH, {
    query: {
      page: query.page ?? 0,
...
export async function fetchCollaboratorTreeDetail(
  ...
  return apiFetch<CollaboratorTreeDetail>(`${COLLABORATOR_TREES_PATH}/${treeId}`, {
    signal,
  })
}
...
export async function deleteCollaboratorTree(treeId: number, signal?: AbortSignal): Promise<void> {
  await apiFetch<void>(`${COLLABORATOR_TREES_PATH}/${treeId}`, {
    method: 'DELETE',
    signal,
  })
}
