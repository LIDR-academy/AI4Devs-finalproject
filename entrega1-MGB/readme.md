...
La experiencia está pensada para usuarios no técnicos: un onboarding paso a paso que evita el uso de prompts libres y guía al dueño del negocio mediante preguntas claras y adaptativas. El producto muestra los resultados en vista previa contextual, lo que facilita la revisión y la toma de decisiones antes de exportar los activos.

### **1.4. Instrucciones de instalación:**

1. Clona el repositorio en tu máquina local.
2. Instala dependencias del backend y frontend según la tecnología del proyecto.
...
- **Interfaz de usuario (GUI):** componente frontend que ofrece el flujo guiado de descubrimiento, editor de perfil y previsualización de activos. Está enfocado en usuarios no técnicos y en minimizar la entrada libre de texto.
- **Discovery Orchestration:** servicio que gestiona el flujo de onboarding, las preguntas adaptativas y el almacenamiento inicial de las respuestas del negocio.
- **Profile Normalization:** servicio que valida, normaliza y enriquece los datos de la empresa. Transforma los insumos en atributos canónicos y categorías estándar.
...
- **Asset Generation:** motor que consume el perfil normalizado y las plantillas para generar contenido con IA. Crea textos para sitio web, descripciones de directorios, biografías sociales y metadatos SEO.
...
- **Frontend / Experiencia de usuario:** gestiona la navegación guiada, la captura de datos del negocio y la visualización de resultados. Este bloque incluye componentes de onboarding, edición de perfil y vista previa de activos, y está diseñado para usuarios no técnicos.
...
#### Historia 1
Título: Capturar el perfil de negocio mediante onboarding guiado
Como Owner
Quiero completar un formulario guiado de onboarding que capture los datos clave de mi negocio
Para disponer de un `BusinessProfile` normalizado y reutilizable para generar activos digitales.

Criterios de aceptación:
- Given que estoy autenticado como Owner, When completo el formulario de onboarding con los campos obligatorios, Then el sistema crea un `BusinessProfile` en estado `draft`.
- Given que el formulario contiene datos incompletos o inválidos, When intento enviar, Then el sistema muestra validaciones y no crea el perfil.
- Given que el Owner confirma la información, When guardo el onboarding, Then el sistema persiste el perfil y registra la acción en `AuditLog`.
...
Criterios de aceptación:
- Given que un `BusinessProfile` existe en estado `draft`, When el sistema ejecuta la normalización, Then crea una `AIRecommendation` con `model_version`, `score` y `explanation`.
- Given que la normalización sugiere cambios en categoría o dirección, When el Admin revisa la recomendación, Then puede aceptar o rechazar la propuesta y el perfil solo avanza a `normalized` tras aprobación humana.
- Given que se aprueba la normalización, When se actualiza el perfil, Then el sistema registra la acción en `AuditLog`.
...
Como Owner
Quiero solicitar la generación de un bundle de activos digitales una vez el perfil esté normalizado
Para obtener contenido listo para revisión sin escribir los textos manualmente.

Criterios de aceptación:
- Given que el `BusinessProfile` está en estado `normalized`, When solicito generación, Then el sistema crea un `AssetPackage` y un `GeneratedAsset` en estado `pending`.
- Given que el worker procesa la tarea async, When la generación termina con éxito, Then el sistema actualiza el `GeneratedAsset` a `ready_for_review` y crea `QualityCheck` y `AIRecommendation`.
- Given que la generación falla por timeout o error de IA, When el worker marca el asset, Then el estado se convierte en `error` o `needs_edit` y se notifica al Owner.
...
Criterios de aceptación:
- Given que hay variaciones disponibles, When el Owner selecciona una variación y edita el contenido, Then el sistema guarda una nueva `AssetVariation` y mantiene la versión anterior.
- Given que edito una variación, When la guardo, Then el cambio queda registrado con `created_by` y `AuditLog`.
- Given que una variación requiere más trabajo, When el Owner guarda, Then el `GeneratedAsset` puede permanecer en `needs_edit` hasta que se apruebe.
...
Criterios de aceptación:
- Given que el `GeneratedAsset` o `AssetVariation` está `approved`, When inicio la publicación, Then se crea un `PublicationTask` en `queued` y emite `publication.requested`.
- Given que el worker procesa la publicación, When la integración externa responde con éxito, Then el `PublicationTask` pasa a `completed` y registra `AuditLog`.
- Given que la publicación falla por credenciales o formato, When el intento termina, Then el sistema actualiza el task a `failed` y notifica con motivo.
...
Como Owner
Quiero ver un dashboard con la tasa de activación y el porcentaje de perfiles con activos
Para medir si el producto está generando resultados y priorizar los próximos pasos.

Criterios de aceptación:
- Given que existen `BusinessProfile`, `GeneratedAsset` y `PublicationTask`, When accedo al dashboard, Then veo métricas de activación, assets publicados y time-to-publish.
- Given que hay datos insuficientes, When ingreso al dashboard, Then se muestra un mensaje claro de que se necesita más actividad para métricas confiables.
- Given que se actualizan los activos, When el dashboard se recarga, Then los valores reflejan los datos actuales del sistema.

Casos borde:
- Si no hay ningún `GeneratedAsset` aprobado, el dashboard debe indicar `0% assets publicados` y no mostrar datos inválidos.
...
Criterios de aceptación:
- Given que el `GeneratedAsset` está en `ready_for_review`, When creo un enlace de revisión, Then el sistema emite `asset.review.shared` y genera un token temporal.
- Given que el colaborador usa el enlace, When accede, Then puede agregar comentarios y estos se registran y notifican al Owner.
- Given que el token expira, When se usa después de la expiración, Then el sistema devuelve 401 y registra el intento en `AuditLog`.
...
- ID: BE-101
- Nombre: Implementar creación de `BusinessProfile` y validaciones de onboarding
- Descripción: Desarrollar el endpoint y la lógica backend para recibir las respuestas del formulario de onboarding, crear un `BusinessProfile` en estado `draft`, validar los campos obligatorios y capturar `gdpr_consent` como requisito para las siguientes etapas.
...
- Descripción: Crear la pantalla/frontend que presenta las recomendaciones de normalización generadas por IA, permite al Admin aceptar o rechazar cambios en categoría, dirección y otros campos clave, y visualiza el estado `draft` vs `normalized`.
...
- Dependencias: Endpoints backend de normalización y aprobación, datos de `BusinessProfile`, esquema de rutas de estado, componente de notificaciones.
- Estimación: 3
