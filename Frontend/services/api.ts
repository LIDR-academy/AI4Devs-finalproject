...
/**
 * Función helper para obtener la configuración de runtime
 */
...
export const apiClient = {
  ...
  async request<T> (endpoint: string, options: RequestInit = {}): Promise<T> {
    // Obtener configuración de runtime cada vez que se hace una petición
    const config = getRuntimeConfig()
    const API_BASE_URL = config.public.apiUrl
    const AUTH_TOKEN_KEY = config.public.authTokenKey
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error(
            `No se pudo parsear respuesta de error de ${endpoint}:`,
            parseError
          )
        }
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        if (!text) {
          console.warn(
            `Respuesta vacía de ${endpoint}, devolviendo objeto vacío`
          )
