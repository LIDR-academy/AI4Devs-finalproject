...
11. **Hand off.** Tell the human the feature is `pr_ready` on branch **`feat/<name>`** (worktree `.worktrees/<name>`); opening & merging the PR is theirs. After merge the worktree can be removed with `git worktree remove .worktrees/<name>`. Append **one terse line** to `progress/history.md` (`date | name | phase | folder | note ≤ 20 words`) — not a paragraph.
