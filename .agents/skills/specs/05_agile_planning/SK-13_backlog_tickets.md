...
1. **Estructura del Backlog y División del Trabajo:**
   - **PASO 0 OBLIGATORIO (Tickets Habilitadores de Core e Infraestructura Monorepo desde Especificaciones):**
     Antes de desglosar historias de usuario, debes analizar la arquitectura definida en `docs/02_architecture_design/` y `AGENTS.md` para extraer dinámicamente el stack tecnológico elegido. Con base en esa documentación, genera obligatoriamente en `docs/05_agile_planning/tickets/shared/`:
     - `shared/backend/TK-001.md`: Ticket técnico de configuración del Core Backend, estructura Monorepo, persistencia, middlewares globales y runner de pruebas alineado estrictamente con el documento de diseño de arquitectura.
     - `shared/frontend/TK-001-FE.md`: Ticket técnico de configuración del Workspace Frontend, sistema de diseño base, tokens HSL, ergonomía táctil (área interactiva $\ge 48\text{px}$) e infraestructura de pruebas UI según las especificaciones de diseño.
   - **PASO 1 (Tickets de Historias de Usuario por Slice Vertical):**
...
2. **Matriz de Trazabilidad del Sprint Backlog:**
   - Una tabla con las columnas: ID Ticket (correlativo, ej. TK-001 para backend shared, TK-001-FE para frontend shared, TK-002, TK-007-B para slices de negocio), ID US Relacionada, Título del Ticket, Módulo/Vertical Slice afectado, Tipo (Backend/Frontend), Estimación de Puntos de Historia (escala Fibonacci, máx. 5 SP) y prioridad MoSCoW.
...
- Tickets Habilitadores de Core: dentro de `docs/05_agile_planning/tickets/shared/backend/TK-001.md` y `docs/05_agile_planning/tickets/shared/frontend/TK-001-FE.md`.
...
Crea un archivo índice que consolide la visualización de todos los tickets en `docs/05_agile_planning/tickets/indice_tickets.md`.
