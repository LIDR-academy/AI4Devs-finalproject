...
- **Name the agent that owns the layer.** `.claude/agents/` holds **`backend-engineer`** (NestJS: domain → application → infrastructure → api), **`frontend-engineer`** (Angular: data-access → feature → ui), **`testing-implementer`** (E2E / API-E2E / acceptance code: `.feature` files and their Cypress step definitions) and **`ci-cd-expert`** (Docker, `docker/`, GitHub Actions workflows, deploy configuration, per-project `project.json` build targets, TypeORM migration squashing). Keep the test handoff split by level: unit tests (`*.spec.ts`, co-located) go with the code to the dev agent that writes it; `.feature` and step-definition work goes to `testing-implementer`. Two skills those agents cite — `frontend-designer` and `i18n-specialist` — do **not** exist in this repository; if a ticket needs that guidance, name the standard it must satisfy rather than a skill that cannot be loaded.
- **The workspace is scaffolding-only.** The Nx workspace, the toolchain and the enforced boundary matrix exist, and so do `apps/api` (NestJS shell) and `apps/web` (Angular shell). There is still **no `libs/` directory**, no domain code, no endpoint and no test to read. So the skill's "read the code first" step is worth doing for anything touching the two application shells or the root configuration, and remains a no-op for every bounded context — say which of the two applies rather than inventing files. Outside those shells every ticket is still greenfield-shaped, and the epic that carries the workspace foundation (`C10`) also carries its scaffolding tickets.
