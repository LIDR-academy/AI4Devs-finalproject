...
class User < ApplicationRecord
  ...
  has_many :vaccinations_administered, class_name: 'Vaccination', foreign_key: 'veterinarian_id', dependent: :nullify
  ...
  # Generate API token for API authentication
  def generate_api_token!
    loop do
      token = SecureRandom.hex(32)
      unless User.exists?(api_token: token)
        update_column(:api_token, token)
        return token
      end
    end
  end
