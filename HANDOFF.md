...
- **BD dev local**: para probar pagos reales hay un cliente del seed con un correo real del desarrollador, porque el sandbox de Flow valida el email del pagador (error 1620 con `@example.cl`). Revertible con un `UPDATE`. Los correos personales no se documentan: quien retome el trabajo debe usar uno propio.
