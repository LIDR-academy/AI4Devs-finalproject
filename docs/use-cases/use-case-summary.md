# Modelo de casos de uso — My Tree Library
...
| **Sistema (My Tree Library)** | Ejecución automática interna (p. ej. envío de correos tras eventos de negocio). |
...
| UC-09 | Notificar por correo a suscriptores | Sistema | N/A | Tras **alta o modificación** de ficha de árbol (según reglas acordadas), informar por e-mail a suscriptores activos. |
...
| **«include»** | UC-03 y UC-04 incluyen UC-09: el flujo de negocio contempla siempre el disparo del proceso de notificación (puede ser no-op si no hay destinatarios). |
...
2. **Alcance de “modificación” que dispara UC-09:** conviene acotar en detalle de implementación qué cambios en ficha (p. ej. solo árboles publicados, o cualquier cambio visible públicamente) generan correo.
