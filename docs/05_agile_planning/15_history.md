...
- **Hallazgo real, encontrado solo al probar la app de punta a punta (Docker real + Playwright real, no lectura de código):** `PinLoginModal.tsx` tenía un `<select>` con 2 operarios de fixtures de desarrollo hardcodeados (`usr-carlos-1`/`usr-maria-2`). En una base de datos de producción nueva, el único usuario real es `bootstrap-admin` (sembrado por `TK-051`) — nunca aparecía en esa lista. Un humano real, sin herramientas de desarrollador, no tenía ninguna forma de loguearse tras un despliegue nuevo, pese a que el backend de bootstrap funciona perfectamente.
...
- **Estado de Tests:** 67/67 frontend (`PinLogin.test.tsx` actualizado para llenar el nuevo campo de ID antes de enviar) + 60/60 backend sin cambios. Build, lint y gate ticket-scoped en verde.
...
- **`TK-056` (nuevo ticket, creado siguiendo Guard 26 antes de tocar código):** `GET /api/v1/auth/users` (rol `ADMIN`) — `IUserRepository.findAll()`, `ListUsersUseCase`, implementado en ambos repositorios (`InMemory`/`Prisma`). Nunca expone `pinHash`. 4 tests nuevos (listado poblado, lista vacía, 403, 401) — 64/64 backend en verde.
- **Frontend:** `UserStatusForm.tsx` reescrito para consumir el listado real — ya no pide el ID del operario por texto, muestra una lista con botón Bloquear/Reactivar por fila. `MovementHistoryPanel.tsx` gana 2 inputs `type="date"` (Desde/Hasta), convertidos a ISO 8601 inicio/fin de día antes de llamar al servicio. 70/70 tests frontend en verde (3 nuevos: listado en `users.service.test.ts`, listado+bloqueo+vacío en `UserManagementPanel.test.tsx`, filtro de fechas en `MovementHistoryPanel.test.tsx`).
...
- **Estado de Build/Lint/Duplicación:** 0 errores; gate ticket-scoped en verde; duplicación 1.81% (bajo el umbral 3%, sin clones nuevos).
