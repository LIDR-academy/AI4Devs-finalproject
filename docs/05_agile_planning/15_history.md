...
- **Estado:**
  - Backend 264 tests / Frontend 145 tests / build / lint verdes. Gates de diff (`check_ticket_code_quality`, `check_dead_code`, `check_ticket_duplication`, `check_decimal_arithmetic`) verdes en cada ticket. `check_migration_schema_parity.sh` (Postgres efímero) confirma que la cadena de migraciones = `schema.prisma`; SQL de `isPristine` verificado también contra Postgres 15 real en transacción con ROLLBACK.
  - **Pendiente:** `TK-104-FE` (pantalla "Cerrar preparación"), `TK-105` / `TK-105-FE` (reporte de mermas + `ConsumeRecipeUseCase` legacy emite `CONSUMPTION_RECIPE`, diferibles). Re-verificación app-level de atomicidad del cierre contra Postgres en vivo → checklist de push (10-sep). `oasdiff` no disponible (breaking-change check no ejecutado).
