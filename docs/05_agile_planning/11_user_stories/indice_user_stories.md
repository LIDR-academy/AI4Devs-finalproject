...
*   **[US-024: Contenido de Ruta Inline y Consistente](shared/US-024.md)**
    *   *Descripción:* Corrige la inconsistencia de `US-023`: Reportes se muestra inline (no como `<Modal>` flotante) y Ajustes pasa a 5 sub-rutas deep-linkables (`/ajustes/configuracion`, `/personal`, `/roles`, `/movimientos`, `/catalogo`). 📋 Approved Spec (`TK-089-FE`, `TK-090-FE`).
