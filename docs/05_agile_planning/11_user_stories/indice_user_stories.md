...
*   **[US-023: Navegación por Rutas y Shell de Aplicación FEFO](shared/US-023.md)**
    *   *Descripción:* Cierra la lámina "Aplicación" del Sistema FEFO: adopta `react-router-dom@7.18.3` (shell de rutas Inventario/Estaciones/Recetas/Reportes/Ajustes con acceso por rol y barra lateral tipo comanda) y los componentes de la propuesta (botón de acción circular, chip de urgencia de 4 niveles, panel Estado de 3 cubetas). ✅ Shell + routing (`TK-085-FE`) implementado; `TK-086-FE`/`TK-087-FE`/`TK-088-FE` (componentes + auditoría contraste) en spec.
