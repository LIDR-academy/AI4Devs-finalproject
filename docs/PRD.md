...
* **Google Calendar payload minimisation**: Event titles sent to Google Calendar contain only the class type and level (e.g., "Group - Intermedio") — no coachee names, coach names, or PII. The association between calendar events and people lives exclusively in PostgreSQL.
