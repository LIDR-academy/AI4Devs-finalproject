...
| Persistencia (exclusiva)    | Spring JDBC + NamedParameterJdbcTemplate (sin JPA/Hibernate) | Built-in                                    |
| Migraciones BD              | Liquibase                                        | 4.x                                                     |
| Autenticación               | Spring Security + OAuth2 Resource Server (JWT · AWS Cognito) | 7.x                                          |
...
| ----------------------- | ------------------------- | ---------------------- | ------------------------------------------------------------------------------ |
| **Portal Certificados** | `certificados.ccb.org.co` | JWT AWS Cognito (obligatorio) | Búsqueda, carrito, liquidación, pago, historial de descargas, descarga de PDFs |
...
### Spring Security con JWT (AWS Cognito)

La autenticación de los endpoints se resuelve con **AWS Cognito** (User Pool) en modo *machine-to-machine*: el frontend/BFF se autentica como aplicación contra Cognito (grant `client_credentials`) y obtiene un access token JWT que el API valida como **OAuth2 Resource Server**. Toda la configuración de Cognito es parametrizable por variables de entorno (issuer, JWK, audiencia y scopes), de modo que cada ambiente (dev/qa/prod) apunta a su propio User Pool sin recompilar.

> **MAUC no interviene en la protección de endpoints.** MAUC se usa exclusivamente como *login SSO del usuario afiliado*: valida la identidad del afiliado dentro del flujo de negocio (claim `username = {tipoDoc}{numDoc}`) pero **no** emite ni valida los tokens de acceso al API. Los módulos de depósitos (OAuth propio) y backoffice (credenciales de servicio) conservan sus mecanismos actuales.
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
            ...
            .oauth2ResourceServer(oauth2 ->
                oauth2.jwt(jwt -> jwt.decoder(cognitoJwtDecoder())
                                     .jwtAuthenticationConverter(cognitoAuthoritiesConverter()))
            )
            .build();
    }

    // Validación de la firma contra el JWKS del User Pool de Cognito
    // + validación de issuer y de audiencia (client-id) por ambiente.
    @Bean
    public JwtDecoder cognitoJwtDecoder(CognitoProperties props) {
        ...
        OAuth2TokenValidator<Jwt> withAudience =
            new JwtClaimValidator<List<String>>("aud",
                aud -> aud != null && aud.contains(props.getClientId()));

        decoder.setJwtValidator(new DelegatingOAuth2TokenValidator<>(withIssuer, withAudience));
        return decoder;
    }

    // Mapea los scopes de Cognito (claim "scope"/"cognito:groups") a authorities
    @Bean
    public JwtAuthenticationConverter cognitoAuthoritiesConverter() {
...
#### Parametrización de Cognito

Los datos del User Pool nunca se codifican en el binario: se enlazan desde configuración/variables de entorno mediante `@ConfigurationProperties`.

```java
@ConfigurationProperties(prefix = "security.cognito")
public record CognitoProperties(
    String region,        // ej. us-east-1
    String userPoolId,    // ej. us-east-1_xxxxxxxxx
    String issuerUri,     // https://cognito-idp.{region}.amazonaws.com/{userPoolId}
    String jwkSetUri,     // {issuerUri}/.well-known/jwks.json
    String clientId,      // app-client autorizado (validación de audiencia)
    List<String> scopes   // scopes esperados (autorización a nivel de endpoint)
) {}
```

```yaml
# application.yml — todos los valores vienen de variables de entorno
security:
  cognito:
...
| Autenticación     | Spring Security OAuth2 Resource Server (JWT AWS Cognito, M2M) |
...
integrations:
  ...
  mauc:                                    # Login SSO de afiliados (NO protege endpoints)
    login-url: ${MAUC_LOGIN_URL}
    tipo-documento-url: ${MAUC_TIPO_DOC_URL}
    signout-url: ${MAUC_SIGNOUT_URL}
  encriptacion:
    url: ${AWS_ENCRYPT_URL}

# Autenticación de endpoints (Resource Server) — AWS Cognito, parametrizable
security:
  cognito:
    region: ${COGNITO_REGION}
    user-pool-id: ${COGNITO_USER_POOL_ID}
...
```sql
-- Lift-and-shift MySQL → dbo (SOAP de generación sin cambios)
...
- JPA/Hibernate está **prohibido** por constitución: toda la persistencia usa Spring JDBC con `NamedParameterJdbcTemplate`
...
- El usuario que solicita un certificado es el mismo que lo descarga (misma sesión, mismo token de Cognito)
