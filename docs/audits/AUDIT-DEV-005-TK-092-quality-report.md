...
| **D-6 / FASE 3** (Media, preexistente) | 🟡 ACEPTADO + TICKET | Abierto **TK-094** (`prisma/migrations/` ↔ `schema.prisma`: faltan `mustChangePin` / `email`). Drift preexistente, ajeno al diff de TK-092. **DoD-4 de TK-092 se acepta como verificado por inspección + estado vivo** (`reconcileOrphanUserRoles` corrió en el stack real: `🔐 Reconciliados 3 usuario(s) sin rol → KITCHEN_STAFF`; 2ª corrida → 0 filas, idempotente; 0 filas `roleId IS NULL` tras el seed). Requiere ✅ del humano. |
...
**Candidatos a Regla Permanente (C-DEV-005-1..3):** pendientes de aprobación humana explícita — NO escritos aún.
