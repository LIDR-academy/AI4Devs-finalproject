...
---
## T01 – Pipeline CI/CD + Infraestructura Local
**Tipo:** DevOps / Fundacional  
**Objetivo:** Contar desde el inicio con un pipeline automatizado que asegure build, test, análisis estático, creación de imagen Docker, migraciones, despliegue a `production`.  
...
2. Merge a `main` publica imagen etiquetada y despliega a `production` automáticamente.
3. Paso de migraciones falla el despliegue si hay error.
4. Ejecutar `docker compose up` levanta todos los servicios y backend arranca aplicando migraciones.
5. Logs muestran versión (commit SHA) al iniciar backend.
