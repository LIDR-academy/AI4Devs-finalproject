...
## Done criteria
- [ ] Scenarios `@s13` (empty column), `@s14` (unequal lengths), `@s15` (malformed pairing → `unavailable` prop) covered by `matching.test.tsx`
- [ ] Unavailable state is non-interactive and does not crash on any of the three triggers
- [ ] Content path (task-3) unchanged for valid slides
