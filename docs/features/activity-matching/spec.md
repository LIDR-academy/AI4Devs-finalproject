...
- **Decision:** Grading lives as a **pure function in `@helsoft/study-buddy`** (`grade-matching.ts`), not a `libs/supabase-services` service/DAO and not co-located in `@helsoft/activities`. — **why:** no I/O (the correct pairing arrives on the slide), so `hooks-service-dao.mdc`'s I/O layers don't apply; `global.mdc` designates study-buddy as the home of business logic; co-locating in the presentational lib would bury domain logic or invert the dependency direction. Mirrors `gradeMultipleChoice`.
