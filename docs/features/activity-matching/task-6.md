...
## Done criteria
- [ ] Scenario `@s16` covered: all matching chrome renders from the active bundle; no hardcoded chrome strings in organism or wrapper
- [ ] All four bundles stay key-aligned (compiler enforces via `TranslationResource`); `summary` interpolation works
- [ ] Localization coverage test (if present) passes for the new keys
- [ ] `pnpm lint` + `pnpm check-types` + `pnpm test` (localization + study-buddy) green
