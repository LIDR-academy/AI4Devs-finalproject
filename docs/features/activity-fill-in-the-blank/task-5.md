...
## Done criteria
- [x] `@s11`/`@s12` covered (empty list **or** any empty-string entry in acceptedAnswers / missing blank → unavailable, no grading)
- [x] `@s6` covered (empty submit → incorrect + resolves + lock)
- [x] Content path (task-3/4) unchanged for valid slides
- [x] `pnpm lint` + `pnpm check-types` + activities/study-buddy tests green
