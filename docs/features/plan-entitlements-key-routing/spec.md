...
## Summary
Client profile flags and generation key routing come from live `plans` rows joined via `profiles.plan_id`. Capability checks never branch on plan name; seeded `free`/`paid` rows set `use_platform_key`, `show_ads`, and `show_key_settings`.

## User stories
- As a free learner, I want clear create gating so I know when generation is available.
...
| Content | `use_platform_key` or saved key (`canCreate`) | Show create/upload; key settings when `show_key_settings` |
| Empty | `!canCreate` (`!use_platform_key` and no key) | Hide create/upload; show contact-support message |
...
- Usage caps, token metering, or quotas (aside from existing platform-generation limits)
...
2. **`public.plans` holds flags; `profiles.plan_id` FKs to it (default `free`).** Seeded free/paid rows; additional plan ids allowed via FK. **Why:** capabilities are data-driven; no `plan === 'paid'` branches.
3. **Plan flag columns:** `use_platform_key`, `show_ads`, `show_key_settings`. **Why:** explicit contract; `canCreate = use_platform_key || hasKey` (no separate `can_create_without_key`).
4. **Use `PLATFORM_GROQ_API_KEY`.** **Why:** provider-explicit naming.
5. **`useProfile()` / `ProfileProvider` → `ProfileService` → `ProfileDao` join; compose `useApiKey()`.** **Why:** layering; one shared profile fetch app-wide.
...
12. **Blocked create UI uses contact-support copy (`upload.cannotCreate`), not ApiKeyRequiredNotice.** **Why:** plan flags decide create; support handles exceptions.
