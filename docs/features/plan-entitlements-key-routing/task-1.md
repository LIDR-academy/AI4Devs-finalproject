...
## Done criteria
- [ ] `profiles.id` is a cascading foreign key to `auth.users.id`
- [ ] `plan` defaults to `free` and rejects every other value
- [ ] Auth-user insert trigger creates exactly one profile
- [ ] RLS permits select-own and no authenticated plan mutation
- [ ] Migration tests cover default, constraint, ownership, and missing-row behavior
- [ ] Scenarios `@s1`, `@s12`, `@s14` are mapped in `tdd.md`
