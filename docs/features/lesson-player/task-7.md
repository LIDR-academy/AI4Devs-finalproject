...
## Done criteria
- [ ] Scenario {s18} covered — from the results slide, Retake returns the deck to the first content slide with no previous in-session answers, results still the final step, session cleared
- [ ] Scenario {s22} covered — after a retake, advancing to results again persists a new attempt (fresh session)
- [ ] `reset` is a pure named reducer transition clearing `currentIndex` + `answers` + `attemptSaved`; no stale state survives
- [ ] Retake performs no navigation/route hop (fully in-deck); `onBackToLessons` routes home
