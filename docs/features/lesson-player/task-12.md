...
## Done criteria
- [x] Scenarios {s4, s10, s15, s16, s20} label/announcement text resolved via `t()` (no hardcoded UI strings)
- [x] All four locale bundles stay key-aligned (`TranslationResource = typeof en` parity — new `player.*` keys in `en` force `es`/`pt`/`de`); locale-parity tests updated if present
- [x] Nav buttons + progress carry accessible names/roles; progress updates announced (WCAG 4.1.3)
