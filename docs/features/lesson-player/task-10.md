...
## Done criteria
- [x] Scenario {s9} covered — resolution failure and missing ref both degrade silently to text-only
- [x] Service maps any signed-URL failure to `null` (no throw reaches the component)
- [x] `SlideImage` renders nothing when `url` is `null`; story includes the failed/degraded case
