...
## Done criteria
- [ ] Scenario {s12} covered — answer an activity, navigate away, return → prior state shown, not reset
- [ ] Scenario {s20} answer-restore leg covered — Back from the results slide shows the last content slide with its answer intact (nav mechanics themselves are task-4)
- [ ] Each wrapper forwards the organism's restore prop (mapping open-ended's stored `submittedAnswer` → `initialSubmittedAnswer`)
- [ ] Restore is derived from the deck's stored `ActivityAnswer` per `slideId`, not duplicated state
- [ ] Wrapper + deck tests updated; `pnpm lint` + `check-types` + `test` green; no hardcoded strings/colors/dimensions
