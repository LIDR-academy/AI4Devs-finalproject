...
## Done criteria
- [ ] Scenario {s16} covered by `lesson-player.test.tsx`; story adds the Error state
- [ ] `useLesson` exposes a `refetch`; Retry re-invokes it and transitions Error → Loading → Content
- [ ] i18n via `t()` (player.error.*); no hardcoded strings/colors/dimensions
