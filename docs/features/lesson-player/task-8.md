...
## Done criteria
- [x] Scenario {s15} covered by `lesson-player.test.tsx`; story adds the Empty state
- [x] Empty is distinct from Error and Loading; no results slide is created for a slideless lesson; Back returns to the lesson detail / list
- [x] i18n via `t()` (player.empty.*); no hardcoded strings/colors/dimensions
