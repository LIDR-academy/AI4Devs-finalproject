...
- Service-layer validation: `libs/supabase-services/src/services/lesson-attempt.service.ts:10-16` rejects empty/whitespace `lessonId`, `total <= 0`, `score < 0`, `score > total` before delegating to the DAO — confirmed landed (not just claimed in `mutation.md`) and pinned by `lesson-attempt.service.test.ts:26-49` (incl. the `score === 0` boundary mutation-kill test).
...
- Spoofing (R3): `NewLessonAttempt` type (`libs/types/src/lesson-attempt.ts:15-19`) carries no `userId` field; DAO insert payload (`libs/supabase-services/src/dao/lesson-attempt.dao.ts:30`) sends only `lesson_id/score/total`; `user_id` is server-set via the column default (`migration:10`) and enforced by RLS `with check`. `lesson-attempt.dao.test.ts:25-35` pins the insert call args to exclude `user_id`, closing off client-side spoofing.
