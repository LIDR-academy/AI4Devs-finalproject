...
## Done criteria
- [ ] @s1 — all system-checked slides answered correctly → `correct === total`.
- [ ] @s2 — a `ScorableSlide[]` mixing multiple-choice/fill-in-the-blank with flashcard/open-ended counts only the system-checked entries (fixtures build directly from `ScorableSlide`, no `Slide` union needed).
- [ ] @s3 — a matching entry contributes exactly one whole-slide point: `isCorrect === true` → +1, `false` (any wrong pair or unpaired item) → +0. Drive purely off `GradedAnswer.isCorrect` (matching's own grader owns the aggregation).
- [ ] @s4 — a system-checked slide with no matching answer counts toward `total` but not `correct`.
- [ ] Fixtures cover all three system-checked `activityType`s (incl. `matching`) even though only `multiple-choice` slides exist today (forward-compat, per risk R6).
- [ ] `total === 0` returns `isScorable: false`; exported via the study-buddy barrel.
- [ ] `pnpm lint` + `pnpm check-types` + `pnpm test` green; short functions, revealing names, no duplication.
