...
## Done criteria
- [ ] Scenario @s14 covered by `use-lesson-generation.test.ts` (mock the service; assert the step sequence + terminal states)
- [ ] Business logic stays in the service; only React state/effects here (per `hooks-service-dao.mdc`)
- [ ] Return shape typed in `use-lesson-generation.types.ts` (exported), not the impl file
- [ ] Exported through the hooks barrel
