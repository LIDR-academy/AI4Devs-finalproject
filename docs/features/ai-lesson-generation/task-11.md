...
## Done criteria
- [ ] Scenarios @s4 (instructional-only → only instructional) / @s5 (activity-only → only activity) / @s6 (composition drives the prompt) covered by unit tests on the pure prompt/assembly modules
- [ ] A parsed deck that violates the composition → rejected (feeds `generation_failed`, task-12) — no wrong-composition deck returned
- [ ] `instructional-only` naturally yields zero activity slides → R7 `ScoreSummary.isScorable === false` (no new flag; risks.md dependency row)
