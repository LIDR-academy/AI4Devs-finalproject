...
## Done criteria
- [x] `get_api_key` is `security definer`, `set search_path = public, vault, pg_temp`, and `execute` is **revoked from public / granted to service_role only** (same model as `save_api_key`/`remove_api_key`) — an `authenticated` client cannot call it even with a valid JWT (@s7)
- [x] Returns the decrypted secret + provider for the given `p_user_id`; returns no row when the user has no key (task-12 maps that to `missing_key`)
- [x] Reversibility note in the migration header (down migration not auto-generated, mirrors R6 migrations)
- [x] Verified against the local Supabase stack (round-trips a Vault secret); manual smoke only — no Jest (Postgres/Deno outside the harness, risks.md R2/R5) — applied via `supabase db push --local`, round-trip + no-row + authenticated-denied/service_role-allowed all confirmed, smoke rows cleaned up afterward
