...
phase: in_review   # pending|spec_ready|approved|in_progress|in_review|mutation|pr_ready|done
review_round: 2     # incremented by reviews_lead; cap 3
---
...
| [task-6](./task-6.md) — Auth error contract (normalization + type) | 2 | @s5,@s6 | todo | libs/services/src/services/auth.service.ts, libs/types/src/auth-error.ts |
| [task-7](./task-7.md) — LoginForm Error + Empty + inline validation | 2 | @s5,@s6,@s8,@s9 | todo | libs/components/src/organisms/login-form/ |
| [task-8](./task-8.md) — i18n keys (auth.*) en/es/pt/de | 3 | @s13 | todo | libs/localization/src/resources/ |
| [task-9](./task-9.md) — a11y pass + Playwright e2e | 3 | @s12 | todo | libs/components/src/organisms/login-form/, libs/components/tests/e2e/ |
