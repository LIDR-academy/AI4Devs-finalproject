...
## Done criteria
- [ ] Scenario @s5 covered: a wrong-email **and** a wrong-password Supabase error both normalize to `invalid_credentials` (single generic outcome — no user enumeration).
- [ ] Scenario @s6 covered: a thrown/network exception normalizes to `network_error`; a subsequent successful call resolves normally (retry works).
- [ ] Service tests assert the mapping against representative supabase-js error shapes and a thrown fetch — the UI never sees a raw error.
- [ ] `AuthErrorCode`/`AuthError` in `@helsoft/types` (`type-name.ts` convention, plain TS), exported via `libs/types/src/index.ts`.
