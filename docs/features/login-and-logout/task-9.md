...
## Done criteria
- [ ] Scenario @s11 covered by `login-form.test.tsx`: accessible labels present, submit has a button role, error node is announced (role/live-region asserted).
- [ ] e2e file at `libs/components/tests/e2e/organisms/login-form/login-form.e2e.js` per the `storybook-e2e-tests` skill (mirrors `src/` path; CommonJS; through `iframe[title="storybook-preview-iframe"]`; text locators). Derive the story URL slug from the `.stories.tsx` `title`/exports.
- [ ] No color-only signaling for errors; touch targets ≥ 44pt verified against tokens.
- [ ] `pnpm lint` + `pnpm check-types` + `pnpm test` green; e2e run non-blocking (`--reporter=list`).
