...
## Done criteria
- [x] @s8: slide with empty front → unavailable notice, nothing interactive, no crash
- [x] @s8: slide with empty back → unavailable notice, nothing interactive, no crash
- [x] The unavailable branch renders before any reveal/self-mark affordance; `onAnswered` never fires
- [x] Covered by `flashcard.test.tsx` (both missing-front and missing-back cases)
- [x] `pnpm lint` + `pnpm check-types` + `pnpm --filter @helsoft/activities test` green
- [x] No hardcoded user-facing strings (unavailable via `t()`)
