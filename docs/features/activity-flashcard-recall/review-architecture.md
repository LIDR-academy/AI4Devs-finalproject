...
- Dependency direction: organism imports only `@helsoft/types`/`components`/`localization`; `flashcard-activity.tsx:1-2` imports `Flashcard` from `@helsoft/activities` (correct downstream). No `@helsoft/services`/`.dao` imports (grep clean). No new package deps.
