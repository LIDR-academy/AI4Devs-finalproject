...
## Done criteria
- [ ] @s9: reveal label, self-mark actions, locked confirmations, answer + explanation headings, and unavailable notice all render from the active locale bundle
- [ ] All four bundles carry the full `activity.flashcard.*` key set (no missing/extra keys; bundles stay key-aligned with `en`)
- [ ] `explanationHeading` reuses the shared "Why" wording precedent (matching/fill-in-the-blank); `unavailable` reuses the shared activity wording
- [ ] `flashcard.tsx` consumes the real `t('activity.flashcard.*')` keys (no placeholder/hardcoded chrome string left in the organism)
- [ ] `libs/activities/src/organisms/flashcard` is added to `KEY_EXISTENCE_DIRS` in `libs/localization/src/coverage/migration-coverage.test.ts` (its own `[name, dir]` entry, mirroring the shipped `multiple-choice`/`matching`/`fill-in-the-blank` activity entries), so every dotted key literal in the organism is asserted to resolve in the `en` bundle
- [ ] `pnpm lint` + `pnpm check-types` + `pnpm --filter @helsoft/localization test` + `pnpm --filter @helsoft/activities test` green
