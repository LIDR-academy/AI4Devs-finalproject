...
## Done criteria
- [x] @s1 hidden: only front visible, back hidden, Reveal shown, no self-mark actions
- [x] @s2 reveal: back becomes visible alongside the front (one-way)
- [x] @s3 revealed: both "Recalled" and "Not recalled" actions available
- [x] @s4 self-mark locks the choice + visually confirms (both `recalled` and not-recalled)
- [x] @s5 no re-mark after lock: tapping the other action does nothing; no new answer
- [x] @s7 explanation displayed on reveal when present; absent (no heading/body) when the slide has none
- [x] `onAnswered` fires exactly once on self-mark; never on reveal; repeat taps ignored
- [x] Split obeys `component-split.mdc` (handlers in `.tsx`, state/derived/effect in the hook, pure transforms already in task-2 helpers); `Component → Hook → Service → DAO` — local `useState` only, no data hook/service/DAO
- [x] `pnpm lint` + `pnpm check-types` + `pnpm --filter @helsoft/activities test` green
- [x] No hardcoded user-facing strings (chrome via `t()`); no hardcoded colors/dimensions (theme tokens)
