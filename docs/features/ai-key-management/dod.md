...
- [x] DTOs not leaked out of data/DAO; barrels updated — `ApiKeyStatus` type confined to `libs/types/src/api-key.ts`; DAO exposes only typed `getApiKeyStatus(): Promise<ApiKeyStatus>` and `removeApiKey()`, never raw key fields. Barrels: `libs/types/src/index.ts`, `libs/supabase-services/src/dao/index.ts`, `libs/supabase-services/src/services/index.ts`, `libs/hooks/src/hooks/index.ts` all updated and tested via `check-types` green.
