...
## Done criteria
- [ ] `libs/types/src/pdf-extraction.ts` exports plain TS types (no framework), one file per `.agents/rules/global.mdc`:
...
- [ ] Re-exported from `libs/types/src/index.ts` barrel.
- [ ] Scenarios @s1/@s2/@s3 are represented in the result shape (per-page ordered text + per-image page/position); no behavior yet, so tests here are type-level/shape assertions where meaningful.
