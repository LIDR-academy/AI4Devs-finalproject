...
## Done criteria
- [ ] `usePdfExtraction()` returns something like `{ extract(file), stage: 'idle'|'processing'|'success'|'error', result, error, reset() }`.
- [ ] Wraps `PdfExtractionService` (never the DAO directly); handles React concerns only.
- [ ] Scenarios @s1 (success → `stage: 'success'`, `result` set) / @s5 (`stage: 'processing'` while in flight) covered by `use-pdf-extraction.test.ts` mocking the service.
- [ ] Exported through `libs/hooks/src/hooks/index.ts`.
