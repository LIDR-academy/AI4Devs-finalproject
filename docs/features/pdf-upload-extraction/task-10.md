...
- [ ] Both read from the single-source `PDF_EXTRACTION_LIMITS` (`maxSizeBytes` = 10 MB, spec decision #1) in `libs/services/src/services/pdf-extraction.constants.ts` and the accepted-type constant — no inline magic numbers.
