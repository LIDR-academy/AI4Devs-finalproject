...
- **Risk:** On Android, the post-answer result banner's assistive-tech announcement relies solely on the banner's `accessibilityLiveRegion` attribute (`multiple-choice.tsx:123-131`). The imperative `AccessibilityInfo.announceForAccessibility` call is intentionally skipped on Android (`Platform.OS !== 'android'` guard, `multiple-choice.tsx:89-93`, commit `38c450b`) to eliminate a confirmed duplicate-announcement bug found in Round 2 (`reviewer_accessibility` verified via RN source that both mechanisms fired together pre-fix). Whether Android's live-region mechanism reliably announces on first mount (as opposed to reacting to a content change on an already-present node) has never been verified on a real device/emulator across any of the 3 review rounds — worst case, Android could get **no** announcement instead of a duplicate one. iOS/web use the imperative call and are fully tested and verified; this residual risk is Android-only.
- **Disposition:** Human-accepted (2026-07-10) at the full-review 3-round cap, per `.agents/rules/review-standards.md` §5 (only a minor remained; no blocker/major; mutation threshold met at 100% on changed logic). Full finding detail and reviewer evidence: `review.md` → "FULL review — Round 3" → "m4-b". Follow-up: an on-device/emulator TalkBack verification pass is recommended but not blocking for this feature.
