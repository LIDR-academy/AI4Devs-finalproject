...
- **No DAO/service — correct, not a gap:** spec's Open decision states grading is pure with no I/O; R7/R9 are
  separate stories that will consume `MultipleChoiceAnswer` later. `grep` for `@helsoft/services`/`dao` across
  changed files → zero. No hook added; `libs/services`/`libs/hooks` untouched.
