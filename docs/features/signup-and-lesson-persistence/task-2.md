...
## Done criteria
- [ ] Scenario(s) {s1, s2, s3} covered by unit tests (persist module + error mapping) on the JS mirror
- [ ] Row persisted before the 200 response; `lessonId` in the response = the DB row id
- [ ] `persist_failed` added to `GenerationErrorCode` + `GENERATION_ERROR_CODES` + `_shared` mirror, mapped to a non-2xx status
- [ ] `_shared/` and `libs/supabase-services` mirror kept in hand-sync (note in file header)
- [ ] `pnpm lint` + `pnpm check-types` + `pnpm test` green
- [ ] No key/body logged; persist uses the caller-JWT client (RLS), never service-role
