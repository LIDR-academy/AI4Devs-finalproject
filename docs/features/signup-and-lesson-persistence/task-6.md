...
## Done criteria
- [x] Scenario(s) {s8, s9, s12, s15, s16} covered by `lessons.dao`/`service` delete tests + `use-lessons.test.ts` + `lesson-list`/`saved-lessons` confirm-dialog tests
- [x] Confirm deletes + removes from list; dismiss keeps the lesson
- [x] Delete scoped to own rows by RLS (no client-side-only filter)
- [x] Delete control + dialog a11y (accessible names, announced); strings from `t()` in en/es/pt/de
