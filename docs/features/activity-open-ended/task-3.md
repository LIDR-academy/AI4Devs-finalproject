...
## Done criteria
- [x] Scenarios `@s1`–`@s4`, `@s10` covered by co-located suites
- [x] Folder has `.tsx` / `.types.ts` / `use-open-ended.ts` / helpers-as-needed + tests per file
- [x] Handlers in component; state/derived/effects in hook; pure in helpers
- [x] `use-open-ended` is UI co-location only (not `@helsoft/hooks` data-layer)
- [x] Enter/return inserts newline and does not call `onSubmit` (@s10)
- [x] Atomic-design + layering (organism depends only on `@helsoft/components`)
- [x] `pnpm lint` + `pnpm check-types` + `pnpm --filter @helsoft/activities test` green
- [x] No hardcoded strings/colors/dimensions (labels + theme)
