...
## Done criteria
- [ ] `@s2`/`@s4`/`@s6` covered by activity tests (reveal path, onAnswered once, shape without `isCorrect`)
- [ ] Assert `isSystemCheckedActivity('open-ended') === false` in @s6 coverage (or type-level note + unit)
- [ ] Invalid slide ⇒ unavailable, no onAnswered (wired fully in task-5)
- [ ] No hook/service/DAO (local useState only for domain answered emission)
