...
## Done criteria
- [x] Scenario `@s6` answered-state shape expressed by the types (submitted-only, no grade field)
- [x] `OpenEndedSlide` matches the spec data contract
- [x] `ActivitySlide` / `ActivityAnswer` unions include the new members; exported via `libs/types` barrel
- [x] `OpenEndedAnswer` does not structurally require / claim `GradedAnswer.isCorrect`
