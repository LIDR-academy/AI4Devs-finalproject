...
>
> Lo que de esa pantalla **todavía no existe**: su **interfaz**. Esta historia es solo el backend — la vista es un marcador de posición deliberado, igual que lo fue la de Usuarios entre su historia de backend y la de interfaz, así que `/taxes/sales-regions` responde pero no pinta ni la tabla, ni el modal de edición, ni los controles de activar/desactivar. Tampoco tiene **entrada en el menú lateral** todavía: se llega escribiendo la URL. Ambas cosas son de la historia siguiente. Y queda anotada como trabajo futuro la red de seguridad a nivel de base de datos para la invariante (una columna generada `STORED` más un índice único, que en MySQL 8.4 la impone de verdad porque los índices únicos ignoran los `NULL`): la transacción con bloqueo de fila es correcta por sí sola, y su orden de escritura —limpiar la antigua antes de marcar la nueva— ya es exactamente el que esa restricción exigiría, así que la historia que la añada añadirá una restricción y no una reescritura.
>
