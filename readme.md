...
> El dominio de ecommerce del PRD está implementado en base de datos **en varias piezas ya**, aunque no en todas: además de autenticación, autorización y ciclo de vida de la cuenta (`users.status`, `users.pending_email`, `users.deleted_at`), existen ya `sales_regions` (el catálogo de regiones fiscales), `media` (la galería de medios compartida), `product_categories`, `products` (con su tabla pivote `product_media` para la galería de cada producto) y `product_sales_region` (la asignación de un producto a las regiones de venta que le aplican) — las **seis** tablas de dominio de la Epic 2 construidas hasta ahora, todas creadas de cero con PK UUID v7. Variantes de producto, pedidos, blog y envíos siguen sin existir en base de datos. Este diagrama se ampliará conforme se construyan esos módulos.
...
>
> **Más allá de las siete del ADR:** `sales_regions` **sí existe ya** y también tiene PK UUID v7, aunque el ADR 0001 no la enumera. Se creó bajo una política de proyecto confirmada que generaliza el ADR (UUID v7 para toda entidad de negocio nueva de la Epic 2, con las tablas internas de geografía de alto volumen exceptuadas y dejadas en `bigint`), y conviene registrarlo con honestidad: el motivo declarado del ADR es evitar la enumeración de identificadores, y un catálogo público y fijo de países no tiene nada que enumerar — así que aquí la clave es por **coherencia** y no por ese razonamiento (un dominio con claves mixtas es peor que una clave sobredimensionada sobre 254 filas). **Enmendar el ADR 0001 para dejar constancia de esa política es una tarea de seguimiento pendiente.** `product_media` y `product_sales_region` quedan fuera de esta cuenta por completo, en ambos sentidos: son tablas pivote sin clave primaria propia —una pareja compuesta de claves foráneas, ya UUID por herencia—, la misma forma que ya usan las tablas pivote de `spatie/laravel-permission`, así que no hay ningún "identificador de entidad" al que aplicarle la política del ADR.
