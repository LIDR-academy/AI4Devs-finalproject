...
>
> Lo que **todavía no existe**: la **vista real** de esa pantalla (tabla, modales y badges de estado son una historia aparte; hoy la vista es un marcador de posición), el CRUD de roles y la asignación de permisos desde el panel, el borrado lógico con ofuscación de email (hoy `delete()` borra de verdad), y el bloqueo de inicio de sesión para las cuentas no activas. El resto de funcionalidades descritas a continuación son el alcance funcional definido en el PRD (`docs/PRD/PRD.md`) y constituyen la hoja de ruta del backoffice.
...
- **Acciones (`app/Actions/**`)** — una carpeta por concern: `Fortify/` implementa los contratos de `laravel/fortify` (registro, reseteo de contraseña); `Users/` contiene las acciones de dominio propias (`RequestEmailChange`, `ConfirmEmailChange`, `CreateUser`, `UpdateUser`). Son clases invocables de un solo propósito, inyectadas por método en el componente o controlador que las usa, y son las que poseen la lógica de escritura: el componente Livewire autoriza y valida, la acción persiste.
...
- **Policies (`app/Policies/`)** — `UserPolicy` es la primera *policy* de la aplicación y responde a la pregunta que un permiso no puede: no "¿puede este actor editar usuarios?" sino "¿puede editar **a este** usuario?". Laravel 13 la descubre automáticamente por nombre (`App\Policies\UserPolicy` ↔ `App\Models\User`), así que no hay `AuthServiceProvider` ni registro manual.
- **Enums, listeners y notificaciones (`app/Enums/`, `app/Listeners/`, `app/Notifications/`)** — `UserStatus` (estado de cuenta), `ActivateVerifiedUser` (único punto de activación, registrado en `AppServiceProvider`), `PendingEmailVerification` (el correo con el enlace firmado) y `UserInvitation` (la invitación a un usuario creado por un administrador).
- **Traducciones (`lang/en/`, `lang/es/`)** — copia de la aplicación separada del código: etiquetas de estado de usuario, textos del flujo de cambio de correo, resumen de la pantalla de usuarios y copia de la invitación.
...
**Autorización por registro concreto (`UserPolicy`).** Un permiso responde "¿puede este actor editar usuarios?"; hay decisiones que exigen responder "¿puede editar **a este** usuario?". `App\Policies\UserPolicy` es la primera *policy* del proyecto y concentra esas reglas: un usuario con el rol `Super Admin` no es editable ni borrable por nadie desde la pantalla; y promover a `Administrator`, degradar a un `Administrator`, borrarlo, suspenderlo o cambiarle el correo exigen además el permiso `roles.manage-administrators` (que ningún rol posee: solo lo ejerce el `Super Admin` por el bypass). La autorización queda así en dos capas complementarias: el middleware de la ruta comprueba el **permiso** de página (`can:users.view`), y dentro del componente un `Gate::authorize()` —primera sentencia de cada método que modifica— consulta la **policy** sobre el registro concreto. Ninguna de las dos sobra, por el motivo que explica el punto siguiente.
