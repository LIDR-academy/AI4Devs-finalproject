...
- **Autenticación y sesión:** login con JWT + refresh token, protección de rutas, guard e interceptor HTTP con bearer.
- **Gestión de casos end-to-end:** creación de casos con `codigo_caso` único, listado paginado con filtros, detalle por `id` y por `codigo_caso`.
- **Catálogos operativos:** países, tipos de identificador (RUT/CC/OTRO), tipos de trabajo, áreas, usuarios y aplicativos.
- **Asignaciones y trazabilidad:** asignación de área y responsable, historial de asignaciones y eventos de auditoría por caso.
- **Ciclo de vida y estados:** transiciones permitidas por flujo, cambio de estado y recorrido histórico de estados del caso.
- **Comunicaciones del caso:** hilo de mensajes/notas internas, envío opcional por WhatsApp y visualización de canal en UI.
- **Kanban por área:** tableros por estados, filtros, drag & drop y movimiento validado por reglas de transición.
- **SLA y alertas:** consulta de SLA por hitos y visualización resumida en tarjeta (semaforización).
- **Imputación de tiempo (CU-15):** registro, edición, eliminación, consulta diaria del usuario y reportes.
- **IA asistida (CU-12):** sugerencias IA por caso y confirmación/rechazo con aplicación controlada.
- **Encuestas post-cierre (CU-09):** envío por token público y reporte de respuestas.
- **Paneles y reportes:** dashboard operativo, reportes de encuestas e imputaciones (incluye exportación Excel).

### **1.3. Diseño y experiencia de usuario:**
La experiencia de usuario del MVP está basada en los mockups definidos para login, layout, crear caso, listado y detalle, y se implementa con componentes Angular orientados a claridad operativa:

- **Login y sesión:** pantalla de acceso con identidad visual TAKUMI/Kibernum, validación de credenciales, mostrar/ocultar contraseña, recuperación de contraseña y protección de rutas con `authGuard`.
- **Navegación y layout:** sidebar + topbar con menú dinámico por permisos, accesos principales (Inicio, Crear caso, Listado), y consistencia de iconografía y estilos.
- **Crear caso:** formulario guiado en bloques (solicitante + caso), validaciones por campo, catálogos dinámicos y retroalimentación clara de errores/éxito.
- **Listado de casos:** filtros, paginación, tabla legible, formato regional de fecha y señales visuales (pills, banderas, estados) para lectura rápida.
- **Detalle de caso:** vista de solo lectura con jerarquía de información, breadcrumbs y estados de carga/error/404 amigables.

En conjunto, el diseño prioriza usabilidad, trazabilidad y velocidad de operación para equipos de soporte y gestión de casos.
...
- **Backend:** estado stateless; sesión vía JWT en cada petición (con flujo de refresh token para renovación de sesión).

---

## 3. Modelo de Datos

> **Nota:** Esta sección fue actualizada con el estado actual del proyecto (según `takumi.md`). El modelo puede seguir evolucionando en próximas iteraciones, pero ya refleja entidades implementadas en backend (casos, ciclo de vida, asignaciones, mensajes, imputación de tiempo, IA, encuestas, SLA y Kanban).
...
| **tipo_identificador** | Tipo de documento del solicitante (RUT, CC, OTRO). | id_tipo_identificador (PK), codigo. | id_tipo_identificador | Contacto N:1. |
    ...
    CrearCasoRequest:
      ...
      properties:
        ...
        numero_contrato_sak:
          type: string
          nullable: true
          description: "Puede ser obligatorio según configuración SAK del entorno."
        id_area_responsable:
          type: integer
          nullable: true
        id_responsable:
...
paths:
  /auth/login:
    post:
      summary: Login (obtener access token + refresh token)
  ...
  /auth/refresh:
    post:
      summary: Renovar sesión con refresh token
      description: Entrega un nuevo access token y refresh token sin pedir usuario/contraseña.
      security: []
      requestBody:
        required: true
        content:
      ...
      responses:
        ...
        "422": { description: "Body inválido (falta refresh_token)" }

  /casos:
    post:
      summary: Crear caso (CU-01, T6-004)
      description: Crea un caso; busca o crea contacto por (id_pais, id_tipo_identificador, identificador), valida RUT/CC según tipo y genera codigo_caso único.
      security: [{ bearerAuth: [] }]
      requestBody:
        required: true
        content:
      ...
      responses:
        ...
        "422": { description: "Validación fallida (identificador inválido, tipo inexistente, reglas SAK, etc.)" }
```
...
```json
{
  ...
  "refresh_token": "eyJ0eXAiOiJKV1QiLCJhbGc...refresh",
  "token_type": "bearer",
...
```json
{
  "access_token": "eyJ0eXAiOiJKV1QiLCJhbGc...nuevo",
  "refresh_token": "eyJ0eXAiOiJKV1QiLCJhbGc...nuevoRefresh",
  "token_type": "bearer",
  "user": { "id": 1, "name": "Demo", "email": "demo@lidr.com" },
  "permissions": ["menu.dashboard", "menu.listado-casos", "menu.crear-caso"]
}
...
**3. POST /api/v1/casos** (crear caso)
...
```json
{
  ...
  "numero_contrato_sak": "CNTR-2026-00123"
}
```
...
- Dado un solicitante con cédula de ciudadanía (CC) con caracteres no numéricos, cuando intento crear el caso, entonces el sistema rechaza con error indicando formato inválido.
...
- **Alcance (pasos):**
  ...
  2. Crear migración `tipo_identificador`: `id` (PK), `codigo`. Seed: RUT, CC, OTRO.
...
- **Precondiciones:** T6-001 completado (catálogos); modelos `Contacto` y `Caso` con validaciones de RUT/CC (T6-002, T6-003) y generación de `codigo_caso` implementados; en caso de no existir aún, el estado inicial del caso puede ser un valor por defecto o placeholder.
- **Alcance (pasos):**
...
- **Criterios de aceptación:** (1) Crear caso con datos válidos devuelve 201 y el caso existe en BD con contacto asociado. (2) RUT inválido → 422. (3) CC con caracteres no numéricos → 422. (4) Tipo de trabajo inexistente → 422.
...
- **Pruebas mínimas:** Feature tests (HTTP): un caso con RUT válido (201 y payload); RUT inválido (422); CC inválida (422); CC válida (201).
...
- **Alcance (pasos):**
  ...
  2. Formulario reactivo con campos: país (select; opciones CL, CO, US), nombre (texto), tipo_identificador (select: RUT, CC), identificador (texto), correo (opcional), teléfono (opcional), tipo_trabajo (select; cargar desde API o estático para MVP), descripción (textarea).
...
**Pull Request 3 — Cierre final del proyecto (entrega definitiva)**

- **Título (sugerido):** `docs: cierre final del proyecto — consolidación README/prompts, evidencia de despliegue y entrega definitiva`
- **Descripción detallada:**
  - **Qué cambia:** Se consolida la documentación final del proyecto para la entrega definitiva: actualización de la ficha y enlaces, arquitectura actual del sistema, especificación de API en formato OpenAPI (3 endpoints principales), detalle de PR2 y cierre de PR3 en el readme; ajuste de `prompts.md` para reflejar el estado de entrega final y los prompts destacados utilizados durante el ciclo completo.
  - **Por qué:** Cumplir los requisitos de la entrega final, dejando trazabilidad completa del proceso (prompts, decisiones, PRs) y documentación técnica/funcional coherente con el estado implementado del sistema TAKUMI.
  - **Impacto:** El repositorio queda listo para evaluación final con plantilla completa (`readme.md` + `prompts.md`), evidencia de despliegue y guía clara para revisión técnica y funcional.
- **Referencia a HU / ticket:** PR de consolidación documental final. Referencia transversal a HU y tickets implementados en PR2 (T6-001 a T6-012, T6-086, T6-087) y a los requisitos de entrega definitiva del curso.
- **Evidencia de despliegue:** `https://aplicaciones.kibernum.com/takumi` (acceso demo indicado en sección 0.4 del readme).
- **Estado:** Entrega final (documentación consolidada + código funcional desplegado).
