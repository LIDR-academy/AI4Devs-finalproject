...
- **Policies (`app/Policies/`)** — `UserPolicy` es la primera *policy* de la aplicación y responde a la pregunta que un permiso no puede: no "¿puede este actor editar usuarios?" sino "¿puede editar **a este** usuario?". `RolePolicy` es la segunda, todavía sin pantalla que la invoque: existe para que el CRUD de roles tenga contra qué autorizar, para que el rechazo del rol `Super Admin` sea efectivo también ahí y —desde la última historia— para exigir el permiso `roles.manage-administrators` al editar o borrar el rol `Administrator` sembrado. Sus tres habilidades son `update`, `delete` y `grantAdministratorPermission` (esta última, cierta solo para el `Super Admin`, es el contrato que consumirá la interfaz de roles para decidir si pinta siquiera el interruptor de ese permiso). Laravel 13 las descubre automáticamente por nombre (`App\Policies\UserPolicy` ↔ `App\Models\User`), así que no hay `AuthServiceProvider` ni registro manual.
...
app/
  ...
  Actions/Roles/      Acciones de dominio de Roles (EnforceAdministratorPermissionGrant)
