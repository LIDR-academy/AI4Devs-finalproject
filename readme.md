...
- **Modelos Eloquent (`app/Models/**`)** — capa de datos; usan atributos PHP 8 (`#[Fillable]`, `#[Hidden]`) y un método `casts()` en vez de las propiedades clásicas `$fillable`/`$hidden`/`$casts`. La *omisión* de una columna en `#[Fillable]` es la propia protección contra asignación masiva: `status` y `pending_email` solo se escriben con `forceFill()` desde una acción concreta. Junto a `User` viven ya `SalesRegion`, `Media`, `ProductCategory` y `Product` (los cuatro modelos del catálogo de la Epic 2, todos con PK UUID v7 desde su creación), más `Role`, subclase del modelo de rol de `spatie/laravel-permission` sobre la **misma** tabla `roles` (sin migración ni columna nueva): es el único modelo de rol que el código de aplicación puede usar, porque es el que lleva las invariantes del `Super Admin` y la identidad centralizada del nivel `Administrator`.
...
> El dominio de ecommerce del PRD está implementado en base de datos **en varias piezas ya**, aunque no en todas: además de autenticación, autorización y ciclo de vida de la cuenta (`users.status`, `users.pending_email`, `users.deleted_at`), existen ya `sales_regions` (el catálogo de regiones fiscales), `media` (la galería de medios compartida), `product_categories`, `products` (con su tabla pivote `product_media` para la galería de cada producto), `product_sales_region` (la asignación de un producto a las regiones de venta que le aplican) y, desde la última historia, `product_attribute_types` + `product_attribute_values` (la taxonomía de atributos de variante que un administrador define — p. ej. "Talla" → 38, 39, 40) — las **ocho** tablas de dominio de la Epic 2 construidas hasta ahora, todas creadas de cero con PK UUID v7. Variantes de producto, pedidos, blog y envíos siguen sin existir en base de datos. Este diagrama se ampliará conforme se construyan esos módulos.
...
>
> **Sigue pendiente:** de las siete entidades con PK UUID que nombra el ADR 0001, ya han llegado `products` y `product_categories` (sin necesitar ninguna enmienda, por estar ya nombradas en el ADR desde el principio); quedan **cuatro** todavía sin código — variantes de producto (Epic 2), y categorías, etiquetas y posts de blog (Epic 4). Nacerán directamente con PK UUID, sin la complejidad de migración anterior por ser tablas nuevas.
>
