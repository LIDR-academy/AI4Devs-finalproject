...
> El dominio de ecommerce del PRD está implementado en base de datos **en varias piezas ya**, aunque no en todas: además de autenticación, autorización y ciclo de vida de la cuenta (`users.status`, `users.pending_email`, `users.deleted_at`), existen ya `sales_regions` (el catálogo de regiones fiscales), `media` (la galería de medios compartida), `product_categories` y `products` (con su tabla pivote `product_media` para la galería de cada producto) — las cuatro tablas de dominio de la Epic 2 construidas hasta ahora, todas creadas de cero con PK UUID v7. Variantes de producto, pedidos, blog y envíos siguen sin existir en base de datos. Este diagrama se ampliará conforme se construyan esos módulos.
