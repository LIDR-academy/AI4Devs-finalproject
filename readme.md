...
[https://github.com/fernme37/AI4Devs-finalproject](https://github.com/fernme37/AI4Devs-finalproject)
...
1. Valor que Entrega (El beneficio que aporta)
...
1. Qué Resuelve (Los problemas que soluciona)
...
1. ¿Para Quién? (El Público Objetivo)
...
1. Motor de Cotización y Cálculo ArquitectónicoCalculadora Integrada de Pendiente y Superficie:
...
1. Logística de Despacho y Entregas ParcialesAsistente de Peso y Cubicaje para Fletes:
...
1. Administración y Roles de Seguridad
...
## Instrucciones Locales De Instalacion Y Ejecucion
...
Modo desarrollo con Vite y `dotnet run`:
...
- `admin@tejaflow.test` / `Admin123!`
- `ventas@tejaflow.test` / `Ventas123!`
- `almacen@tejaflow.test` / `Almacen123!`

Guia detallada: `[docs/local-development.md](docs/local-development.md)`.
...
- Uso con PrimeVue: Se usarán componentes nativos de PrimeVue (como el  para listar las tejas por lote y color, y el  para capturar las mermas en el patio). TypeScript asegura que los objetos recibidos de la API de .NET coincidan exactamente en sus tipos de datos (evitando dolores de cabeza con valores null o vacíos).
...
- Las vistas del panel de administración importan los componentes avanzados de PrimeVue (como el  para auditar las mermas o buscar tipos de tejas), y se estilizan de manera responsiva utilizando CSS3.
...
- Cómo funciona: El usuario inicia sesión y el servidor le devuelve un token de corta duración. El frontend de Vue.js almacena este token de forma segura y lo adjunta automáticamente en cada petición HTTP en la cabecera Authorization: Bearer .
...
- En la Interfaz Web (XSS - Cross-Site Scripting): El motor de renderizado de Vue.js 3 escapa automáticamente cualquier texto que pongas entre llaves ({{ teja.nombre }}). Si un atacante registra un cliente con el nombre , Vue lo imprimirá literalmente en pantalla como texto inofensivo en lugar de ejecutar el script.
...
builder.Services.AddCors(options => {
    options.AddPolicy("ProdPolicy", policy => {
        policy.WithOrigins("[https://tejaflow.com](https://tejaflow.com)") // Solo se permite tu frontend oficial
              .AllowAnyMethod()
              .AllowAnyHeader();
    });
});
...
- url: [https://tejaflow.compaths](https://tejaflow.compaths):
...
- El componente  restringe de forma nativa que se introduzcan valores flotantes o menores a 1.
...
**Implementación End-to-End de TejaFlow**

Este PR implementa la primera versión funcional completa de **TejaFlow**, pasando de la especificación inicial a una aplicación ERP operativa con backend .NET, frontend Vue, persistencia en SQL Server, autenticación por roles, pruebas automatizadas, contenedores Docker y preparación de despliegue CI/CD hacia Oracle Cloud.

**Resumen**

Se construye la base técnica y funcional del sistema: inventario por lotes, registro de mermas, cotizaciones arquitectónicas, ventas con pagos, afectación transaccional de inventario, logística de entregas parciales y dashboard administrativo. Además, se agrega infraestructura para ejecución local, SQL Server en Docker, pruebas backend, imágenes Docker, GitHub Actions, secretos documentados y despliegue automático.

**Alcance Principal**

- Backend ASP.NET Core .NET 10 con arquitectura por capas.
- Frontend Vue 3 + TypeScript + PrimeVue.
- SQL Server 2022 con EF Core migrations y seed data.
- JWT authentication y autorización por roles.
- Módulos de inventario, ventas, cotizaciones, logística y administración.
- Docker Compose local y producción.
- Pipeline CI/CD con GitHub Actions y GHCR.
- Documentación de ejecución local, secretos y despliegue Oracle.

**Validación**

Se verificó el flujo completo con Docker:

- Login con usuarios seed.
- Consulta de inventario.
- Cotización.
- Ventas con efectivo, tarjeta de crédito y tarjeta de débito.
- Descuento de inventario por venta.
- Registro de merma.
- Despacho parcial.
- Restricciones de rol.
- Frontend por Nginx y proxy `/api`.
- `dotnet test`: 6 pruebas exitosas.
