...
class MediaTreePhotosDeleteServiceTest {
  ...
  @InjectMocks private MediaTreePhotosDeleteService service;

  @Test
  void deleteAllPhotosForTree_noPhotos_skipsStorageAndRepositoryDelete() {
    Jwt jwt = Jwt.withTokenValue("t").header("alg", "none").subject("sub").build();
    when(catalogMediaPermissionClient.resolveActorUsuarioAppIdForTree(5L, jwt)).thenReturn(42L);
    when(fotografiaRepository.findAllByArbolId(5L)).thenReturn(List.of());

    service.deleteAllPhotosForTree(5L, jwt);

    verify(objectStorageRemover, never()).removeObject(org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.any());
    verify(fotografiaRepository, never()).deleteAll(org.mockito.ArgumentMatchers.any());
  }

  @Test
  void deleteAllPhotosForTree_removesObjectsThenMetadata() {
    Jwt jwt = Jwt.withTokenValue("t").header("alg", "none").subject("sub").build();
    when(catalogMediaPermissionClient.resolveActorUsuarioAppIdForTree(5L, jwt)).thenReturn(42L);

    Fotografia photo = new Fotografia();
    photo.setFotografiaId(10L);
    photo.setBucketAlmacenamiento("mtl-photos");
    photo.setClaveObjeto("trees/5/a.jpg");
    when(fotografiaRepository.findAllByArbolId(5L)).thenReturn(List.of(photo));
  ...
  @Test
  void deleteAllPhotosForTree_storageFailure_abortsBeforeMetadataDelete() {
    Jwt jwt = Jwt.withTokenValue("t").header("alg", "none").subject("sub").build();
    when(catalogMediaPermissionClient.resolveActorUsuarioAppIdForTree(5L, jwt)).thenReturn(42L);
    ...
    assertThatThrownBy(() -> service.deleteAllPhotosForTree(5L, jwt))
