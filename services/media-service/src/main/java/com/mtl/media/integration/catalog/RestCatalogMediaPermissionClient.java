...
public class RestCatalogMediaPermissionClient implements CatalogMediaPermissionClient {
  ...
  @Override
  public long resolveActorUsuarioAppIdForTree(long treeId, Jwt jwt) {
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND) {
        throw new ResponseStatusException(
            HttpStatus.NOT_FOUND, "No existe el árbol indicado en el catálogo.");
      }
      if (ex.getStatusCode() == HttpStatus.FORBIDDEN) {
        throw new ResponseStatusException(
            HttpStatus.FORBIDDEN, "No tiene permiso para asociar fotografías a este árbol.");
      }
      throw new ResponseStatusException(
