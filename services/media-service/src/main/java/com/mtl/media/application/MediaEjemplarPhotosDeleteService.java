...
@Service
public class MediaEjemplarPhotosDeleteService {
  ...
  public MediaEjemplarPhotosDeleteService(
  ...
  /**
   * Elimina todas las fotografías del ejemplar (metadatos y objetos en bucket). Requiere permiso de
   * colaborador/admin sobre el ejemplar (misma política que subida, HU-006).
   */
  @Transactional
  public void deleteAllPhotosForEjemplar(long ejemplarId, Jwt jwt) {
    catalogMediaPermissionClient.resolveActorUsuarioAppIdForEjemplar(ejemplarId, jwt);

    List<Fotografia> photos = fotografiaRepository.findAllByEjemplarId(ejemplarId);
    if (photos.isEmpty()) {
      return;
    }
  ...
  private void deleteObjectQuietly(String bucket, String objectKey) {
    try {
      ...
      throw new MediaStorageException(
          "No se pudieron eliminar las fotografías del ejemplar en el almacén de objetos.", ex);
    }
  }
}
