...
/**
 * Postgres + Kafka (Docker): consume {@code catalog.ejemplar.evento}, idempotencia por {@code
...
class NotificationEjemplarCreadoKafkaIT {

  private static final String CONSUMER_GROUP = "notification-ejemplar-it-" + UUID.randomUUID();
  ...
  void limpiarYCrearSuscriptorActivo() {
    ...
    s.setAltaEn(OffsetDateTime.now(ZoneOffset.UTC));
    suscriptorRepository.saveAndFlush(s);
  }

  @Test
  void kafka_ejemplarCreado_persisteEventoNotificacionYEnvio() throws Exception {
    long eventoId = 8_001_100L + (System.nanoTime() % 1_000_000);
    long ejemplarId = 50_001L;
    String json = payloadJson(eventoId, ejemplarId);

    kafkaTemplate.send("catalog.ejemplar.evento", String.valueOf(ejemplarId), json).get();
  ...
  @Test
  void kafka_ejemplarCreado_segundaEntrega_idempotenteSinDuplicarNotificacion() throws Exception {
    long eventoId = 8_002_200L + (System.nanoTime() % 1_000_000);
    long ejemplarId = 50_002L;
    String json = payloadJson(eventoId, ejemplarId);
  ...
  private String payloadJson(long eventoId, long ejemplarId) {
    return """
        {
          "evento_id": %d,
