 ...
 * email}, {@code nombrePerfil}) proceden del JWT validado; el resto alinea con {@code catalog.ejemplar}.
 */
public record CreateEjemplarCommand(
    String subjectOidc,
    Long especieId,
    Long provinciaId,
    BigDecimal latitud,
  ...
  public static CreateEjemplarCommand fromRequest(CreateEjemplarRequest request, OidcUserProfile profile) {
    return new CreateEjemplarCommand(
        profile.subject(),
        request.speciesId(),
        request.provinceId(),
        request.latitude(),
