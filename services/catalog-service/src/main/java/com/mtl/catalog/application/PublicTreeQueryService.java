...
@Service
public class PublicTreeQueryService {
  ...
  private static final Set<String> SORT_FIELDS =
      Set.of("especie", "provincia", "municipio", "estado", "visibilidad", "treeId");
  private static final Set<String> SORT_DIRECTIONS = Set.of("asc", "desc");

  private final PublicTreeReadRepository publicTreeReadRepository;

  public PublicTreeQueryService(PublicTreeReadRepository publicTreeReadRepository) {
    this.publicTreeReadRepository = publicTreeReadRepository;
  }

  @Transactional(readOnly = true)
  public PublicTreePageResponse listPublishedTrees(
      int page,
      int size,
      String sort,
      PublicTreeFilters filters,
      Jwt jwt) {
    ...
    Page<PublicTreeListItemDto> results =
        publicTreeReadRepository
            .findPublicTreeRows(
    ...
    return new PublicTreePageResponse(
        results.getContent(),
  ...
  @Transactional(readOnly = true)
  public PublicTreeDetailDto getPublishedTreeDetail(long treeId, Jwt jwt) {
    AccessScope scope = resolveScope(jwt, null, null);
    PublicTreeDetailRow row =
        publicTreeReadRepository
            .findPublicTreeDetailRow(treeId, scope.estado(), scope.visibilidad())
            .orElseThrow(
                () ->
                    new CatalogNotFoundException(
                        "No se encontró el árbol público solicitado con id " + treeId));

    return new PublicTreeDetailDto(
        row.getTreeId(),
        row.getNombreComun(),
        row.getNombreCientifico(),
        row.getProvincia(),
  ...
  public record PublicTreeFilters(
