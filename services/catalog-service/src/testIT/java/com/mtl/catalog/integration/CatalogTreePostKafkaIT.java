...
/**
 * Postgres + Kafka (Docker). POST alta de árbol y un mensaje {@code ARBOL_CREADO} en
 * {@code catalog.arbol.evento}.
 */
@Tag("integration")
@Testcontainers
...
class CatalogTreePostKafkaIT {
  ...
  void createConsumer() {
    ...
    consumer.subscribe(Collections.singletonList("catalog.arbol.evento"));
  }
  ...
  @Test
  void postTree_persisteYEmiteArbolCreadoEnKafka() throws Exception {
    String body =
        """
        {
          "speciesId": %d,
    ...
    String eventJson = pollUntilArbolCreado(treeId);
    var node = jsonMapper.readTree(eventJson);
    assertThat(node.path("tipo_evento").toString().replace("\"", "")).isEqualTo("ARBOL_CREADO");
    assertThat(node.path("arbol_id").asLong()).isEqualTo(treeId);
    assertThat(node.path("evento_id").asLong()).isPositive();
    assertThat(node.hasNonNull("ocurrido_en")).isTrue();
  }

  private String pollUntilArbolCreado(long expectedArbolId) {
    ...
    while (System.nanoTime() < deadline) {
      ...
      for (var r : records) {
        ...
        try {
          ...
          if ("ARBOL_CREADO".equals(n.path("tipo_evento").toString().replace("\"", ""))
              && n.path("arbol_id").asLong() == expectedArbolId) {
            return r.value();
          }
    ...
    throw new AssertionError("No se recibió ARBOL_CREADO para arbol_id=" + expectedArbolId + " a tiempo");
  }
}
