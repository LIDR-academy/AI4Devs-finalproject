...
describe('isSystemCheckedActivity', () => {
  it.each(SYSTEM_CHECKED_ACTIVITY_TYPES)('returns true for the system-checked type "%s"', (activityType) => {
    expect(isSystemCheckedActivity(activityType)).toBe(true);
  });

  it.each(['flashcard', 'open-ended'] as const)('returns false for the excluded type "%s"', (activityType) => {
