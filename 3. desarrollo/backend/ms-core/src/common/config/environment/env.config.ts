...
const envSchema = joi.object({
    ...
    logFormat: joi.string().valid('json', 'text').default('text'),

    // Throttle Configuration
    throttleMaxRequestsPerIp: joi.number().min(1).default(100),
    throttleBanDuration: joi.number().min(1000).default(60000),
}).unknown(true);
...
export const envs = {
    ...
    throttle: {
        maxRequestsPerIp: envVars.throttleMaxRequestsPerIp,
        banDuration: envVars.throttleBanDuration,
    },
    circuitBreaker: {
        timeout: envVars.circuitBreakerTimeout,
        errorThresholdPercentage: envVars.circuitBreakerErrorThresholdPercentage,
        resetTimeout: envVars.circuitBreakerResetTimeout,
    },
};
