import { RasamEntity, RasamParams } from "../domain/entity";
import { RasamPort } from "../domain/port";
import { RasamValue } from "../domain/value";

export class RasamUseCase implements RasamPort {
  constructor(private readonly repository: RasamPort) { }

  public async findAll(params: RasamParams): Promise<{ data: RasamEntity[]; total: number; }> {
    try {
      const listed = await this.repository.findAll(params);
      return listed;
    } catch (error) {
      throw error;
    }
  }

  public async findById(id: number): Promise<RasamEntity | null> {
    try {
      const geted = await this.repository.findById(id);
      return geted;
    } catch (error) {
      throw error;
    }
  }

  public async create(data: RasamEntity): Promise<RasamEntity | null> {
    try {
      const value = new RasamValue(data).toJson();
      const created = await this.repository.create(value);
      return created;
    } catch (error) {
      throw error;
    }
  }

  public async update(id: number, data: RasamEntity): Promise<RasamEntity | null> {
    try {
      await this.findById(id);
      const value = new RasamValue(data);
      const updated = await this.repository.update(id, value);
      return updated;
    } catch (error) {
      throw error;
    }
  }

  public async delete(id: number): Promise<RasamEntity | null> {
    try {
      await this.findById(id);
      const deleted = await this.repository.delete(id);
      return deleted;
    } catch (error) {
      throw error;
    }
  }
}

