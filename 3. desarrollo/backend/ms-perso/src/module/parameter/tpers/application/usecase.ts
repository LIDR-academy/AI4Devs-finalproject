import { TpersEntity, TpersParams } from "../domain/entity";
import { TpersPort } from "../domain/port";
import { TpersValue } from "../domain/value";

export class TpersUseCase implements TpersPort {
  constructor(private readonly repository: TpersPort) { }

  public async findAll(params: TpersParams): Promise<{ data: TpersEntity[]; total: number; }> {
    try {
      const listed = await this.repository.findAll(params);
      return listed;
    } catch (error) {
      throw error;
    }
  }

  public async findById(id: number): Promise<TpersEntity | null> {
    try {
      const geted = await this.repository.findById(id);
      return geted;
    } catch (error) {
      throw error;
    }
  }

  public async create(data: TpersEntity): Promise<TpersEntity | null> {
    try {
      const value = new TpersValue(data).toJson();
      const created = await this.repository.create(value);
      return created;
    } catch (error) {
      throw error;
    }
  }

  public async update(id: number, data: TpersEntity): Promise<TpersEntity | null> {
    try {
      await this.findById(id);
      const value = new TpersValue(data);
      const updated = await this.repository.update(id, value);
      return updated;
    } catch (error) {
      throw error;
    }
  }

  public async delete(id: number): Promise<TpersEntity | null> {
    try {
      await this.findById(id);
      const deleted = await this.repository.delete(id);
      return deleted;
    } catch (error) {
      throw error;
    }
  }
}

