...
2. **code implementation**: 
    ...
    The code implementation for API calls must be robust and handle all possible scenarios, including edge cases and error handling. Use exception handling to manage errors gracefully and provide meaningful error messages to users. To support reliability and resilience when interacting with external services, incorporate the following libraries as appropriate:
    - `pybreaker` to ensure circuit breaker pattern implementation
    - `tenacity` to implement retry logic for transient errors
    Flask views should remain synchronous by default. Apply `async`/`await` selectively in handlers that perform **concurrent I/O‑bound operations** (for example, multiple external calls aggregated with `asyncio.gather`) when doing so delivers measurable benefit. Background work and long‑running processing belong in Celery tasks, not in asyncio-based request handlers.
