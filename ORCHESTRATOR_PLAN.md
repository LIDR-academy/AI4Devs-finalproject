...
> **Decisions locked in:** orchestrator lives under `.agents/` (extends existing folder) · mutation testing uses **StrykerJS** · pipeline driven by an **orchestrator agent** (`orchestrator_lead`) with a single human approval gate.
...
| Build | `tdd_craftsman` strict TDD | Code Agent by vertical slice | **`tdd_craftsman`**, strict TDD **by vertical slice** (1→2→3), branching by artifact type (UI vs logic), always integration tests |
| Review | single `judge` | `/arch` + `/security` separately | **6 parallel reviewers** (code, design, architecture, security, accessibility, performance) + a **`reviews_lead`** that consolidates all findings and issues one change request to the craftsman |
...
│   ├── tdd_craftsman.md          # Phase 2 (+ change re-work in Phase 3)
...
└── features/
    └── <name>/                   # ONE folder per feature — everything for it lives here
        ...
        ├── tdd.md                # tdd_craftsman — TDD cycle log + @scenario → test map
...
flowchart TD
    ...
    GATE -->|"approved"| P3G
    subgraph P3G["② tdd_craftsman — strict TDD, one vertical slice at a time (in_progress)"]
        direction LR
    ...
    RL -->|"CHANGES_REQUESTED (consolidated)"| P3G
    RL -->|"all 6 APPROVED → mutation"| MUT["④ mutation_tester<br/>StrykerJS on changed files"]
    MUT -->|survivors| P3G
    MUT -->|"threshold met"| DV["⑤ dod_validator<br/>validate full DoD → dod.md"]
    DV -->|DOD_FAILED| P3G
...
### Phase 2 — `tdd_craftsman` (Build, strict TDD)
...
- **Feature gate → `in_review` (after all slices):** every `@s` covered by ≥1 concrete test across the slices; the integration test green; `pnpm --filter <ws> test` and relevant `test:e2e` green; `pnpm lint` and `pnpm check-types` green; no scope beyond the contract; no hardcoded strings/colors/dims. Craftsman does **not** self-mark `done`. (The parallel review round and mutation run once per feature, after all slices are green — not per slice, to keep the heavy gates affordable. For unusually large features the lead may run the review round per slice.)
- **Re-work loop:** in Phase 3, `reviews_lead` sends the craftsman one consolidated change request per round; in Phase 4, `orchestrator_lead` sends surviving mutants. Either way `tdd_craftsman` writes the failing test that captures each gap, makes it green, and returns for re-review.
...
3. Verdict: if **all six are APPROVED** → report `APPROVED` up to `orchestrator_lead` → `mutation`. If **any** requested changes → issue **one consolidated change request** to `tdd_craftsman` (it never edits code itself).
4. Re-review: after `tdd_craftsman` fixes via TDD, `reviews_lead` re-runs **all six reviewers in parallel** again (a full round — cheap to re-run, avoids a fix breaking another dimension) and re-consolidates.
5. **Round cap:** at most **3 review rounds**; on a 3rd round still carrying changes, `reviews_lead` stops and escalates to the human (recorded in `review.md`) rather than ping-ponging indefinitely.

No fixed ordering among reviewers — they are independent lenses, so running them concurrently is both faster and avoids a false "mechanics first" sequencing. `reviews_lead` is the single point that turns six parallel opinions into one actionable request for the craftsman.
...
- **Loop:** any survivor → back to `tdd_craftsman` (write the red test that kills it) → re-review path as needed → re-run Stryker. Only when the threshold is met does the feature advance to `dod_validator`.
...
- **Behavior:** run the **complete DoD** (Section 7) against the implemented feature and write a pass/fail report to `docs/features/<name>/dod.md` — every DoD item marked `[x]`/`[ ]` with concrete evidence (command output, `file:line`, links to `review.md` / `mutation.md`). It re-runs the objective checks itself (lint, types, unit/integration/e2e suites, mutation threshold) rather than trusting prior reports. If any item fails, it returns `DOD_FAILED` and `orchestrator_lead` routes the gap back to `tdd_craftsman`; it never patches code itself.
...
7. Write the 12 role files in `.agents/agents/` (orchestrator_lead + spec_partner + tdd_craftsman + reviews_lead + 6 reviewers + mutation_tester + dod_validator), each with frontmatter, tools, protocol, gates, and hard rules — ported from the reference agents but adapted to this stack.
...
**Suggested build order:** Phase 0 → AGENTS.md + rules → agents (lead first, then spec/gherkin, then craftsman, then reviewers, then mutation, then pr) → command → dry run.
...
- **Figma access — RESOLVED:** no Figma in this repo. `tdd_craftsman` builds UI from the spec, or from a pasted screenshot if the story includes one. No Figma MCP step.
...
- **Reviewer loop termination — RESOLVED:** each reviewer loops at most **3 times**; the 3rd `CHANGES_REQUESTED` escalates to the human (see Phase 3, step 5).
