...
**Incluye:** selector de jugadores, cálculo de baraja/rondas, creación de borrador local en estado `setup`, vista previa de configuración y navegación al paso siguiente (añadir jugadores — LPT-6).
...
**Alineación Firestore futura** (`data-model.md`): al sincronizar, mapear a `games/{gameId}` con `status: setup`, `maxPlayers`/`playerCount`, y subcolección `rounds` pre-generada o embebida en `settings.roundSequence` *(definir en implementación)*.
